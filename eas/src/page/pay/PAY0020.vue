        ...
        <div class="box-field">
          ...
          <div class="app-box-style ">
              ...
              <div class="upload-buttons">
                ...
                <button type="button" :class="logButtonClass" class="applicant-btn success w-20" v-if="timekeeping.clientPayGroupId  && timekeeping.cutOffStartDate && timekeeping.cutOffEndDate && isSchedule>0" @click="onManualUpload">            
                <i class="fa fa-sticky-note-o fa-lg"></i><span class="bold">Batch Entry</span> 
                </button>
...
<div class="board p-1 mb-0 w-100">
  ...
  <div class="d-flex justify-centerXXX fixed-header table-container "  v-show="excelData">
    <table class="striped-even mb-0 table-modal">
      <thead>
        <tr>
          ...
          <th class="text-center w-40">Member Name</th>
          <th class="text-center w-10">Date</th>
          <th class="text-center w-40">Schedule</th>
        </tr>
      </thead>
      <tbody class="white">
          <tr v-for="(dtl, index) in excelData" :key="index" :class="{ 'danger-light bold': dtl.invalidFlag }">
            ...
            <td class="text-center w-40">{{ dtl.memberName}}</td>
            <td class="text-center w-10">{{ dtl.scheduleDate}}</td>
    ...
    <td colspan="7" class="text-center warning-light bold">
      Total Schedule Count: {{ excelData.length }} 
    </td>
  </tr>
  </tfoot>
    </table> 
   ...
   <div class="buttons d-flex justify-center mt-3 " shadow-soft mb-0>
    <!-- <div class="left-buttons d-flex gap-2">
    <button type="button" class="success border-main" v-show="isCopyExcelData" @click="onSelectAllData()">
      <i class="fa fa-check mr-2"></i>Select All
    </button>
    <button type="button" class="danger border-main" v-show="isCopyExcelData" @click="onDeselectAllData()">
      <i class="fa fa-close mr-2"></i>Deselect All
    </button>
  </div> -->

  
    <button type="button" class="info border-main " @click="onSubmitExcelData()">
      <i class="fa fa-check mr-2 "></i>Submit
    </button>
    <button type="button" class="warning border-main mr-0" @click="isExcelDataVisible=false">
  ...
  <div class=" fixed-header table-container border"  v-show="excelData">
    <table class="striped-even mb-0 table-modal ">
      <thead>
        <tr>
          <th class="text-center w-5">#</th>
          <th class="text-center w-5">Member ID</th>
          <th class="text-center w-90">Member Name</th>
      ...
      <tbody class="white p-0">
          <tr v-for="(dtl, index) in allMembers" :key="index" :class="{ 'danger-light bold': dtl.invalidFlag }">
            ...
            <td class="text-center w-10">
               <input type="checkbox" class="d-flex" v-model="dtl.selectFlag" />
     ...
     <tr>
    <td colspan="7" class="text-center warning-light bold">
        Total Member Count: {{ selectedMembersCount  }} 
    </td>
  </tr>
  </tfoot>
    </table> 
...
<div class="board p-1 mb-0 w-100">
  <form id="pay0020A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
    <div class="schedule-editor-boxes gap">
      <sym-date  v-model="manualSchedule.startScheduleDate" caption="Start Date" align="bottom"></sym-date>
      <sym-date  v-model="manualSchedule.endScheduleDate" caption="End Date" align="bottom"></sym-date>

      <sym-combo  v-model="manualSchedule.noScheduleId"  align="bottom" :caption-width="46" caption="No Schedule" display-field="noScheduleName" :datasource="noSchedules"/>
      
      </div>
    <div class="schedule-editor-boxes-2 gap">
      
      <sym-combo v-if="visibleCombos >= 1" v-model="manualSchedule.timekeepingScheduleId" align="bottom" :caption-width="46" caption="Schedule" display-field="scheduleName" :datasource="scheduleList"/>
      <sym-combo v-if="visibleCombos >= 2" v-model="manualSchedule.timekeepingScheduleIdA" align="bottom" :caption-width="46" caption="Schedule B" display-field="scheduleName" :datasource="scheduleListA"/>  
      <sym-combo v-if="visibleCombos >= 3" v-model="manualSchedule.timekeepingScheduleIdB" align="bottom" :caption-width="46" caption="Schedule C" display-field="scheduleName" :datasource="scheduleListB"/>
      <sym-combo v-if="visibleCombos >= 4" v-model="manualSchedule.timekeepingScheduleIdC" align="bottom" :caption-width="46" caption="Schedule D" display-field="scheduleName" :datasource="scheduleListC"/>
      <sym-combo v-if="visibleCombos >= 5" v-model="manualSchedule.timekeepingScheduleIdD" align="bottom" :caption-width="46" caption="Schedule E" display-field="scheduleName" :datasource="scheduleListD"/>
    </div>

    


   <div class="btns   " shadow-soft mb-0>
    <div class="buttons w-100 justify-left " fw-26 shadow-soft mb-0>
      <button @click="addComboManual" :disabled="visibleCombos >= maxCombos" class="success-light w-30"><i class="fa fa-plus mr-2"></i>Add Schedule</button>
      <button @click="removeComboManual" :disabled="visibleCombos <= 1" class="danger-light"><i class="fa fa-minus mr-2"></i>Remove</button>
    </div>
...
<!-- 
    ...
    <button type="button" class="success border-main" @click="onSelectAllData()">
      <i class="fa fa-check mr-2"></i>Select All
    </button>
    <button type="button" class="danger border-main" @click="onDeselectAllData()">
      <i class="fa fa-close mr-2"></i>Deselect All
    </button>
  ...
  <div class="d-flex justify-centerXXX fixed-header table-container " >
    <table class="striped-even mb-0 table-modal">
      <thead class="border">
        <tr>
          <th class="text-center w-5">#</th>
          <th class="text-center w-5">Member ID</th>
          <th class="text-center w-80">Member Name</th>
          <!-- <th class="text-center w-10">Include</th> -->
              <th class="w-10 text-center">
              <input
                type="checkbox"
                :checked="isAllSelected"
      ...
      <tbody class="white">
          <tr v-for="(dtl, index) in allMembers" :key="index" :class="{ 'danger-light bold': dtl.invalidFlag }">
            ...
            <td class="text-center w-5">{{ index + 1 }}</td>
            <td class="text-center w-5">{{ dtl.memberId}}</td>
            <td class="text-center w-80">{{ dtl.memberName}}</td>
            <td class="text-center w-10">
               <input
    type="checkbox"
    class="d-flex "
    v-model="dtl.selectFlag" 
  />
            </td>
            <!-- <td class="text-center w-15">{{ dtl.scheduleDate}}</td>
            <td class="text-center w-40">{{ dtl.scheduleCode}}</td> -->
...
export default {
 ...
 toggleSelectAll(event) {
    ...
    this.allMembers.forEach(member => {
      member.selectFlag = isChecked;
    });
  },

onSubmitManualSchedule() {

if (!this.allMembers.some(member => member.selectFlag)) {
  console.warn('No members selected');
  this.advice.fault('No members selected to update.');
  return;
}

  if (!this.manualSchedule || !this.manualSchedule.startScheduleDate || !this.manualSchedule.endScheduleDate) {
    console.fault('Missing start or end schedule dates');
    this.advice.fault('Please provide start and end schedule dates.');
    return;
  }
  console.log('Raw startScheduleDate:', this.manualSchedule.startScheduleDate);
  console.log('Raw endScheduleDate:', this.manualSchedule.endScheduleDate);

  const startDate = new Date(
    this.manualSchedule.startScheduleDate.year,
    this.manualSchedule.startScheduleDate.month - 1,
    this.manualSchedule.startScheduleDate.day
  );
  const endDate = new Date(
    this.manualSchedule.endScheduleDate.year,
    this.manualSchedule.endScheduleDate.month - 1,
    this.manualSchedule.endScheduleDate.day
  );
  ...
  if (startDate > endDate) {
    ...
    this.advice.fault('Start date cannot be after end date.');
    return;
  }

  const membersToUpdate = this.allMembers.filter(member => member.selectFlag);
  ...
  for (let dt = new Date(startDate); dt <= endDate; dt.setDate(dt.getDate() + 1)) {
    const scheduleDate = formatDate(dt);
    // console.log('Processing date:', scheduleDate);

    membersToUpdate.forEach(member => {
      // console.log(`Checking schedule for member ${member.memberId} on ${scheduleDate}`);

      const existingIndex = this.schedules.findIndex(
        sch => String(sch.memberId) === String(member.memberId) && sch.scheduleDate === scheduleDate
      );

      if (existingIndex !== -1) {
        console.log(`Updating existing schedule at index ${existingIndex}`);
      } else {
        console.log('No existing schedule found, adding new one');
      }

      // Build scheduleCodeList from manualSchedule timekeepingScheduleId* properties
      let scheduleCodeList = '';
      const scheduleIds = [
        this.manualSchedule.timekeepingScheduleId,
        this.manualSchedule.timekeepingScheduleIdA,
        this.manualSchedule.timekeepingScheduleIdB,
        this.manualSchedule.timekeepingScheduleIdC,
        this.manualSchedule.timekeepingScheduleIdD
      ].filter(id => id !== 0 && id !== null && id !== undefined);

      console.log('scheduleIds:', scheduleIds);

      scheduleIds.forEach(id => {
        const match = this.scheduleList.find(s => s.timekeepingScheduleId == id);
        if (match) {
          scheduleCodeList += (scheduleCodeList ? ' | ' : '') + match.scheduleName;
        }
      });

      console.log('scheduleCodeList:', scheduleCodeList);

const noScheduleId = this.manualSchedule.noScheduleId;
// console.log('noScheduleId:', noScheduleId);

const noScheduleName = noScheduleId === 2 ? 'NO SCHEDULE' : noScheduleId === 1 ? 'REST DAY' : '';
// console.log('noScheduleName:', noScheduleName);

const scheduleCode = noScheduleName
  ? (scheduleCodeList ? noScheduleName + ' | ' + scheduleCodeList : noScheduleName)
  : scheduleCodeList;

// console.log('Final scheduleCode:', scheduleCode);
      ...
      const newSchedule = {
        ...
        noScheduleId: this.manualSchedule.noScheduleId,
        timekeepingScheduleId: this.manualSchedule.timekeepingScheduleId,
        timekeepingScheduleIdA: this.manualSchedule.timekeepingScheduleIdA,
        timekeepingScheduleIdB: this.manualSchedule.timekeepingScheduleIdB,
        timekeepingScheduleIdC: this.manualSchedule.timekeepingScheduleIdC,
        timekeepingScheduleIdD: this.manualSchedule.timekeepingScheduleIdD,
  ...
  this.advice.success('Manual schedules updated successfully.', { duration: 5 });
  // this.clearSchedulePad();

    this.isManualScheduleEditorVisible = false;
   },

    onShowManualScheduleEditor () {
      const me = this;

      me.setActiveModel('manualSchedule');
   ...
   addComboManual() {
    if (this.manualSchedule.timekeepingScheduleId==999999999) {
      return ; 
    }
  ...
  if (this.manualSchedule[currentField]) {
  ...
  removeComboManual() {
    if (this.visibleCombos > 1) {
      ...
      this.manualSchedule[fieldToClear] = null;
  ...
  onManualUpload() {
    ...
    console.log("clearSchedulePad" , this.clearExcelSchedulePad())
    this.clearExcelSchedulePad();
    this.manualSchedule.startScheduleDate = this.timekeeping.cutOffStartDate;
    this.manualSchedule.endScheduleDate = this.timekeeping.cutOffEndDate;
    
    this.allMembers = Object.values(
    this.schedules.reduce((acc, schedule) => {
    if (!acc[schedule.memberId]) {
      acc[schedule.memberId] = {
        memberId: schedule.memberId,
        memberName: schedule.memberName,
        selectFlag: false, // true if found
        schedules: []
      };
      }
  
      acc[schedule.memberId].schedules.push(schedule);
  ...
  onSelectAllData() {
  this.allMembers.forEach(member => {
    member.selectFlag = true;
  });
},

  onDeselectAllData() {
  this.allMembers.forEach(member => {
    member.selectFlag = false;
  });
},
 ...
 onCopyExcelData() {
  ...
  // this.allMembers.forEach(schedule => {
  ...
  reader.onload = (e) => {
    ...
    rawData.forEach(row => {
      ...
      if (this.excelData.length==0) {
        me.advice.fault("No data found.", { duration: 5} );
        return; 
      }
       this.isExcelDataVisible = true;
...
// Step 2: Build a Set of memberIds from validSchedules for quick lookup
const validMemberIds = new Set(validSchedules.map(item => String(item.memberId)));

// Step 3: Group schedules by memberId and set selectFlag accordingly
this.allMembers = Object.values(
  this.schedules.reduce((acc, schedule) => {
    if (!acc[schedule.memberId]) {
      acc[schedule.memberId] = {
        memberId: schedule.memberId,
        memberName: schedule.memberName,
        selectFlag: validMemberIds.has(String(schedule.memberId)), // true if found
        schedules: []
      };
    }
    acc[schedule.memberId].schedules.push(schedule);
...
// Step 4: Log the result for debugging
...
safeFormatDate(date) {
    ...
    clearExcelSchedulePad () {
      const d = this.manualSchedule;

      d.startScheduleDate= null,
      d.endScheduleDate= null,
      d.noScheduleId = 0,
      d.noScheduleId = 0;
      d.timekeepingScheduleId = 0;
      d.timekeepingScheduleIdA = 0;
 ...
 isAllSelected() {
    return Array.isArray(this.allMembers) && this.allMembers.length > 0 &&
      this.allMembers.every(member => member.selectFlag);
  },

  selectedMembersCount() {
    if (!Array.isArray(this.allMembers)) return 0;
    return this.allMembers.filter(member => member.selectFlag).length;
  },

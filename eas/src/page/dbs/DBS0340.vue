...
<sym-form id="dbs0340" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 ">
      <thead>
        <tr class="align-top">
          ...
          <th class="w-40">Allowance Name</th>
      ...
      <tbody class="white">
        <tr v-for="(dtl, index) in allowances" :key="index">
          <td data-label="Allowance ID" class="text-right">{{ dtl.allowanceId }}</td>
          <td data-label="Allowance Name">{{ dtl.allowanceName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!allowances.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0340A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.allowanceId" caption="Allowance ID" align="bottom"></sym-int>
        <sym-text v-model="detail.allowanceName" caption="Allowance Name" align="bottom"></sym-text>
       ...
       <sym-check v-model="detail.payHourly" caption="Hourly" align="bottom" :caption-width="0" caption-align="center" check-align="center"></sym-check>
        <sym-int v-model="detail.payMaxAmount" caption="Maximum Amount" align="bottom"></sym-int>
...
export default {
  ...
  methods: {
    ...
    payTrxCodeCallback (e) {
      ...
      let index = me.allowances.findIndex(obj => obj.payTrxCode === e.proposedValue);
      if (index > -1) {
    ...
    createRecord () {
      ...
      return ajax('api/allowances/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/allowances/' + this.detail.allowanceId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/allowances/' + detail.allowanceId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.allowances.findIndex(obj => obj.allowanceId === success[0].allowanceId);
            if (index > -1) {
              me.allowances[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.allowances.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.allowanceId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('allowanceName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.allowanceId = dtl.allowanceId;
      d.allowanceName = dtl.allowanceName;
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsAllowance', me.detail.allowanceId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Allowance <b>' + me.allowances[index].allowanceName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.allowances[me.recordIndex];

        dtl.allowanceId = d.allowanceId;
        dtl.allowanceName = d.allowanceName;
...
.record-editor-boxes {
  display: grid;
  grid-template-columns: 1.5fr 4fr 4fr 1fr 1fr 2fr 1fr;
  gap: .5rem;
}

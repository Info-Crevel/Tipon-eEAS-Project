...
<sym-form id="dbs0410" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
  ...
  <div class="d-flex justify-center fixed-header" ref="list">
    <table class="striped-even mb-0 w-50">
      ...
      <tbody class="white">
        <tr v-for="(dtl, index) in languages" :key="index">
          <td data-label="Language ID" class="text-right">{{ dtl.languageId }}</td>
          <td data-label="Language Name">{{ dtl.languageName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!languages.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0410A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.languageId" caption="Language ID" align="bottom"></sym-int>
        <sym-text v-model="detail.languageName" caption="Language Name" align="bottom"></sym-text>
...
export default {
  ...
  methods: {
    ...
    createRecord () {
      ...
      return ajax('api/languages/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/languages/' + this.detail.languageId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/languages/' + detail.languageId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.languages.findIndex(obj => obj.languageId === success[0].languageId);
            if (index > -1) {
              me.languages[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.languages.push(success[0]);
    ...
    onEditRecord (dtl, index) {
      ...
      d.languageId = dtl.languageId;
      d.languageName = dtl.languageName;
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsLanguage', me.detail.languageId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Language <b>' + me.languages[index].languageName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.languages[me.recordIndex];

        dtl.languageId = d.languageId;
        dtl.languageName = d.languageName;

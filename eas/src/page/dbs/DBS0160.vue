// Applicant Position Setup

<template>
<section class="container p-0" :key="ts">
<sym-form id="dbs0160" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-50">
      <thead>
        <tr>
          ...
          <th class="w-50">Applicant Position Name</th>
          <th class="w-14 text-right">Sort Seq</th>
          <th class="w-26">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in applicantPositions" :key="index">
          <td data-label="Applicant Position ID" class="text-right">{{ dtl.applicantPositionId }}</td>
          <td data-label="Applicant Position Name">{{ dtl.applicantPositionName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!applicantPositions.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0160A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.applicantPositionId" caption="Applicant Position ID" align="bottom"></sym-int>
        <sym-text v-model="detail.applicantPositionName" caption="Applicant Position Name" align="bottom"></sym-text>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add Applicant Position';
      }
      return 'Edit Applicant Position';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getApplicantPositionList()
        .then( list => {
          me.stopWait(wait);
          me.applicantPositions = list;
    ...
    getApplicantPositionList () {
      return get('api/applicant-positions');
    },
    ...
    createRecord () {
      ...
      return ajax('api/applicant-positions/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/applicant-positions/' + this.detail.applicantPositionId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/applicant-positions/' + detail.applicantPositionId + this.getDeleteQuery(detail.lockId), options);
    },
    ...
    onLoad () {
      ...
      dc.keyField = 'applicantPositionId';
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.applicantPositions.findIndex(obj => obj.applicantPositionId === success[0].applicantPositionId);
            if (index > -1) {
              me.applicantPositions[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.applicantPositions.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.applicantPositionId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('applicantPositionName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.applicantPositionId = dtl.applicantPositionId;
      d.applicantPositionName = dtl.applicantPositionName;
    ...
    onDeleteRecord (index) {
      const
        me = this,
        d = me.detail,
        dtl = me.applicantPositions[index];
      ...
      getSafeDeleteFlag('DbsApplicantPosition', me.detail.applicantPositionId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Applicant Position <b>' + me.applicantPositions[index].applicantPositionName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.applicantPositions.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.applicantPositions[me.recordIndex];

        dtl.applicantPositionId = d.applicantPositionId;
        dtl.applicantPositionName = d.applicantPositionName;
    ...
    clearRecordPad () {
      ...
      d.applicantPositionId = 0;
      d.applicantPositionName = '';
      d.sortSeq = 0;
      d.lockId = '';
    }

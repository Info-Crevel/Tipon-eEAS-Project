// Inventory Release Charging

<template>
<section class="container w-48" :key="ts">
<sym-form id="dbs0330" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-100">
      <thead>
        <tr>
          ...
          <th class="w-43">Inventory Charge Name</th>
          <th class="w-10 text-center">Client</th>
          <th class="w-13 text-center">Member</th>
          <th class="w-11 text-right">Sort Seq</th>
          <th class="w-17">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in inventoryCharges" :key="index">
          <td data-label="Inventory Charge ID" class="text-right">{{ dtl.inventoryChargeId }}</td>
          <td data-label="Inventory Charge Name">{{ dtl.inventoryChargeName }}</td>
          <td data-label="Client" class="text-center">
            <i :class="getBooleanIconClass(dtl.clientFlag)"></i>
          </td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!inventoryCharges.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0330A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.inventoryChargeId" caption="Charge ID" align="bottom"></sym-int>
        <sym-text v-model="detail.inventoryChargeName" caption="Inventory Charge Name" align="bottom"></sym-text>
        <sym-check v-model="detail.clientFlag" caption="Client" align="bottom" :caption-width="0" caption-align="center" check-align="center"></sym-check>
        <sym-check v-model="detail.memberFlag" caption="Member" align="bottom" :caption-width="0" caption-align="center" check-align="center"></sym-check>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add Inventory Charge';
      }
      return 'Edit Inventory Charge';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getInventoryChargeList()
        .then( list => {
          me.stopWait(wait);
          me.inventoryCharges = list;
    ...
    getInventoryChargeList () {
      return get('api/inventory-charges');
    },
    ...
    createRecord () {
      ...
      return ajax('api/inventory-charges/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/inventory-charges/' + this.detail.inventoryChargeId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/inventory-charges/' + detail.inventoryChargeId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.inventoryCharges.findIndex(obj => obj.inventoryChargeId === success[0].inventoryChargeId);
            if (index > -1) {
              me.inventoryCharges[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.inventoryCharges.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.inventoryChargeId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('inventoryChargeName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.inventoryChargeId = dtl.inventoryChargeId;
      d.inventoryChargeName = dtl.inventoryChargeName;
      d.clientFlag = dtl.clientFlag;
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsInventoryCharge', me.detail.inventoryChargeId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Inventory Charge <b>' + me.inventoryCharges[index].inventoryChargeName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.inventoryCharges.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.inventoryCharges[me.recordIndex];

        dtl.inventoryChargeId = d.inventoryChargeId;
        dtl.inventoryChargeName = d.inventoryChargeName;
        dtl.clientFlag = d.clientFlag;
    ...
    clearRecordPad () {
      ...
      d.inventoryChargeId = 0;
      d.inventoryChargeName = '';
      d.clientFlag = false;
      d.memberFlag = false;
...
.record-editor-boxes {
  display: grid;
  grid-auto-flow: column;
  grid-template-columns: 5fr 12fr 1fr 1fr 3fr;
  gap: .5rem;
}

...
<sym-form id="dbs0090" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-60">
      <thead>
        <tr class="align-top">
          ...
          <th class="w-43">Education Level Name</th>
          <th class="w-10 text-center">Course</th>
          <th class="w-10 text-center">Upload</th>
          <th class="w-11 text-right">Sort Seq</th>
          <th class="w-20 action">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in educationLevels" :key="index">
          <td data-label="Education Level ID" class="text-right">{{ dtl.educationLevelId }}</td>
          <td data-label="Education Level Name">{{ dtl.educationLevelName }}</td>
          <td data-label="Course" class="text-center">
            <i :class="getBooleanIconClass(dtl.courseNameRequiredFlag)"></i>
          </td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!educationLevels.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0090A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.educationLevelId" caption="Education Level ID" align="bottom"></sym-int>
        <sym-text v-model="detail.educationLevelName" caption="Education Level Name" align="bottom"></sym-text>
        <sym-check v-model="detail.courseNameRequiredFlag" caption="Course" align="bottom" :caption-width="0" caption-align="center" check-align="center"></sym-check>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add Education Level';
      }
      return 'Edit Education Level';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getEducationLevelList()
        .then( list => {
          me.stopWait(wait);
          me.educationLevels = list;
    ...
    getEducationLevelList () {
      return get('api/education-levels');
    },
    ...
    createRecord () {
      ...
      return ajax('api/education-levels/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/education-levels/' + this.detail.educationLevelId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/education-levels/' + detail.educationLevelId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.educationLevels.findIndex(obj => obj.educationLevelId === success[0].educationLevelId);
            if (index > -1) {
              me.educationLevels[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.educationLevels.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.educationLevelId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('educationLevelName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.courseNameRequiredFlag = dtl.courseNameRequiredFlag;
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsEducationLevel', me.detail.educationLevelId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Education Level <b>' + me.educationLevels[index].educationLevelName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.educationLevels.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.educationLevels[me.recordIndex];

        dtl.educationLevelId = d.educationLevelId;
        dtl.educationLevelName = d.educationLevelName;
        dtl.courseNameRequiredFlag = d.courseNameRequiredFlag;
    ...
    clearRecordPad () {
      ...
      d.courseNameRequiredFlag = false;

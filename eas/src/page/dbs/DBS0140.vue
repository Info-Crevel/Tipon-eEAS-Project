// Source of Application Setup

<template>
<section class="container p-0" :key="ts">
<sym-form id="dbs0140" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-50">
      <thead>
        <tr>
          ...
          <th class="w-50">Application Source Name</th>
          <th class="w-14 text-right">Sort Seq</th>
          <th class="w-26">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in applicationSources" :key="index">
          <td data-label="Application Source ID" class="text-right">{{ dtl.applicationSourceId }}</td>
          <td data-label="Application Source Name">{{ dtl.applicationSourceName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!applicationSources.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0140A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.applicationSourceId" caption="Application Source ID" align="bottom"></sym-int>
        <sym-text v-model="detail.applicationSourceName" caption="Application Source Name" align="bottom"></sym-text>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add Application Source';
      }
      return 'Edit Application Source';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getApplicationSourceList()
        .then( list => {
          me.stopWait(wait);
          me.applicationSources = list;
    ...
    getApplicationSourceList () {
      return get('api/application-sources');
    },
    ...
    createRecord () {
      ...
      return ajax('api/application-sources/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/application-sources/' + this.detail.applicationSourceId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/application-sources/' + detail.applicationSourceId + this.getDeleteQuery(detail.lockId), options);
    },
    ...
    onLoad () {
      ...
      dc.keyField = 'applicationSourceId';
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.applicationSources.findIndex(obj => obj.applicationSourceId === success[0].applicationSourceId);
            if (index > -1) {
              me.applicationSources[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.applicationSources.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.applicationSourceId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('applicationSourceName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.applicationSourceId = dtl.applicationSourceId;
      d.applicationSourceName = dtl.applicationSourceName;
    ...
    onDeleteRecord (index) {
      const
        me = this,
        d = me.detail,
        dtl = me.applicationSources[index];
      ...
      getSafeDeleteFlag('DbsApplicationSource', me.detail.applicationSourceId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Application Source <b>' + me.applicationSources[index].applicationSourceName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.applicationSources.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.applicationSources[me.recordIndex];

        dtl.applicationSourceId = d.applicationSourceId;
        dtl.applicationSourceName = d.applicationSourceName;
    ...
    clearRecordPad () {
      ...
      d.applicationSourceId = 0;
      d.applicationSourceName = '';
      d.sortSeq = 0;
      d.lockId = '';
    }

// Bank Definition Setup

<template>
<section class="container w-38" :key="ts">
<sym-form id="dbs0290" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-100">
      <thead>
        <tr>
          ...
          <th class="w-55">Bank Name</th>
          <th class="w-13 text-right">Sort Seq</th>
          <th class="w-22">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in banks" :key="index">
          <td data-label="Bank ID" class="text-right">{{ dtl.bankId }}</td>
          <td data-label="Bank Name">{{ dtl.bankName }}</td>
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0290A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.bankId" caption="Bank ID" align="bottom"></sym-int>
        <sym-text v-model="detail.bankName" caption="Bank Name" align="bottom"></sym-text>
...
export default {
  ...
  methods: {
    ...
    createRecord () {
      ...
      return ajax('api/banks/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/banks/' + this.detail.bankId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/banks/' + detail.bankId + this.getDeleteQuery(detail.lockId), options);
    },
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsBank', me.detail.bankId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Bank <b>' + me.banks[index].bankName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {

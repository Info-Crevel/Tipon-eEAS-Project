// CDA Membership Type Setup

<template>
<section class="container p-0" :key="ts">
<sym-form id="dbs0400" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-50">
      <thead>
        <tr>
          ...
          <th class="w-30">CDA Member Type Name</th>
      ...
      <tbody class="white">
        <tr v-for="(dtl, index) in cdaMemberTypes" :key="index">
          <td data-label="CDA Member Type ID" class="text-right">{{ dtl.cdaMemberTypeId }}</td>
          <td data-label="CDA Member Type Name">{{ dtl.cdaMemberTypeName }}</td>
          <td data-label="Amount" class="text-right">{{ core.toDecimalFormat(dtl.cdaMemberTypeAmount) }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!cdaMemberTypes.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0400A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.cdaMemberTypeId" caption="CDA Member Type ID" align="bottom"></sym-int>
        <sym-text v-model="detail.cdaMemberTypeName" caption="CDA Member Type Name" align="bottom"></sym-text>
        <sym-dec v-model="detail.cdaMemberTypeAmount" caption="Amount" align="bottom" captionAlign="right" :blankZero="false"></sym-dec>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add CDA Member Type';
      }
      return 'Edit CDA Member Type';
    }
  },
  ...
  methods: {
    loadData () {
      ...
      me.getCDAMemberTypeList()
        .then( list => {
          me.stopWait(wait);
          me.cdaMemberTypes = list;
    ...
    getCDAMemberTypeList () {
      return get('api/cda-member-types');
    },
    ...
    createRecord () {
      ...
      return ajax('api/cda-member-types/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/cda-member-types/' + this.detail.cdaMemberTypeId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/cda-member-types/' + detail.cdaMemberTypeId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.cdaMemberTypes.findIndex(obj => obj.cdaMemberTypeId === success[0].cdaMemberTypeId);
            if (index > -1) {
              me.cdaMemberTypes[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.cdaMemberTypes.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.cdaMemberTypeId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('cdaMemberTypeName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.cdaMemberTypeId = dtl.cdaMemberTypeId;
      d.cdaMemberTypeName = dtl.cdaMemberTypeName;
      d.cdaMemberTypeAmount = dtl.cdaMemberTypeAmount;
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsCDAMemberType', me.detail.cdaMemberTypeId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('CDA Member Type <b>' + me.cdaMemberTypes[index].cdaMemberTypeName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.cdaMemberTypes.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.cdaMemberTypes[me.recordIndex];

        dtl.cdaMemberTypeId = d.cdaMemberTypeId;
        dtl.cdaMemberTypeName = d.cdaMemberTypeName;
        dtl.cdaMemberTypeAmount = d.cdaMemberTypeAmount;

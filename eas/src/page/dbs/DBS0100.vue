...
<sym-form id="dbs0100" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-50">
      <thead>
        <tr>
          ...
          <th class="w-50">Disability Name</th>
          <th class="w-14 text-right">Sort Seq</th>
          <th class="w-26">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in disabilities" :key="index">
          <td data-label="Disability ID" class="text-right">{{ dtl.disabilityId }}</td>
          <td data-label="Disability Name">{{ dtl.disabilityName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!disabilities.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0100A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.disabilityId" caption="Disability ID" align="bottom"></sym-int>
        <sym-text v-model="detail.disabilityName" caption="Disability Name" align="bottom"></sym-text>
...
export default {
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getDisabilityList()
        .then( list => {
          me.stopWait(wait);
          me.disabilities = list;
    ...
    createRecord () {
      ...
      return ajax('api/disabilities/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/disabilities/' + this.detail.disabilityId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/disabilities/' + detail.disabilityId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.disabilities.findIndex(obj => obj.disabilityId === success[0].disabilityId);
            if (index > -1) {
              me.disabilities[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.disabilities.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.disabilityId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('disabilityName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.disabilityId = dtl.disabilityId;
      d.disabilityName = dtl.disabilityName;
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsDisability', me.detail.disabilityId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Disability <b>' + me.disabilities[index].disabilityName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.disabilities.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.disabilities[me.recordIndex];

        dtl.disabilityId = d.disabilityId;
        dtl.disabilityName = d.disabilityName;

// Revenue Qualification Setup

<template>
<section class="container p-0" :key="ts">
<sym-form id="dbs0430" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-50">
      <thead>
        <tr>
          ...
          <th class="w-50">Revenue Qualification Name</th>
          <th class="w-14 text-right">Sort Seq</th>
          <th class="w-26">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in revenueQualifications" :key="index">
          <td data-label="Revenue Qualification ID" class="text-right">{{ dtl.revenueQualificationId }}</td>
          <td data-label="Revenue Qualification Name">{{ dtl.revenueQualificationName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!revenueQualifications.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0430A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.revenueQualificationId" caption="Rev Qualification ID" align="bottom"></sym-int>
        <sym-text v-model="detail.revenueQualificationName" caption="Revenue Qualification Name" align="bottom"></sym-text>
...
export default {
  ...
  data () {
    return {
      revenueQualifications: [],

      detail: {
        revenueQualificationId: 0,
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add Revenue Qualification';
      }
      return 'Edit Revenue Qualification';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getRevenueQualificationList()
        .then( list => {
          me.stopWait(wait);
          me.revenueQualifications = list;
    ...
    getRevenueQualificationList () {
      return get('api/revenue-qualifications');
    },
    ...
    createRecord () {
      ...
      return ajax('api/revenue-qualifications/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/revenue-qualifications/' + this.detail.revenueQualificationId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/revenue-qualifications/' + detail.revenueQualificationId + this.getDeleteQuery(detail.lockId), options);
    },
    ...
    onLoad () {
      ...
      dc.keyField = 'revenueQualificationId';
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.revenueQualifications.findIndex(obj => obj.revenueQualificationId === success[0].revenueQualificationId);
            if (index > -1) {
              me.revenueQualifications[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.revenueQualifications.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.revenueQualificationId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('revenueQualificationName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.revenueQualificationId = dtl.revenueQualificationId;
      d.revenueQualificationName = dtl.revenueQualificationName;
    ...
    onDeleteRecord (index) {
      const
        me = this,
        d = me.detail,
        dtl = me.revenueQualifications[index];
      ...
      getSafeDeleteFlag('DbsRevenueQualification', me.detail.revenueQualificationId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Revenue Qualification <b>' + me.revenueQualifications[index].revenueQualificationName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.revenueQualifications.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.revenueQualifications[me.recordIndex];

        dtl.revenueQualificationId = d.revenueQualificationId;
        dtl.revenueQualificationName = d.revenueQualificationName;
    ...
    clearRecordPad () {
      ...
      d.revenueQualificationId = 0;
      d.revenueQualificationName = '';
      d.sortSeq = 0;
      d.lockId = '';
    }

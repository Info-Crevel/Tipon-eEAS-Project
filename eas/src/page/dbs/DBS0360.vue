...
<sym-form id="dbs0360" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-50">
      <thead>
        <tr>
          ...
          <th class="w-50">Affiliation Name</th>
      ...
      <tbody class="white">
        <tr v-for="(dtl, index) in affiliations" :key="index">
          <td data-label="Affiliation ID" class="text-right">{{ dtl.affiliationId }}</td>
          <td data-label="Affiliation Name">{{ dtl.affiliationName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!affiliations.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0360A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.affiliationId" caption="Affiliation ID" align="bottom"></sym-int>
        <sym-text v-model="detail.affiliationName" caption="Affiliation Name" align="bottom"></sym-text>
...
export default {
  ...
  methods: {
    ...
    createRecord () {
      ...
      return ajax('api/affiliations/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/affiliations/' + this.detail.affiliationId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/affiliations/' + detail.affiliationId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.affiliations.findIndex(obj => obj.affiliationId === success[0].affiliationId);
            if (index > -1) {
              me.affiliations[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.affiliations.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.affiliationId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('affiliationName');
              }, 100);
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsAffiliation', me.detail.affiliationId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Affiliation <b>' + me.affiliations[index].affiliationName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.affiliations[me.recordIndex];

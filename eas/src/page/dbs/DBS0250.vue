...
<template>
<section class="container w-42  " :key="ts">
<sym-form id="dbs0250" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-100">
      <thead>
        <tr>
          ...
          <th class="w-56">Insurance Coverage Name</th>
          <th class="w-14 text-right">Sort Seq</th>
          <th class="w-20">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in insuranceCoverages" :key="index">
          <td data-label="Insurance Coverage ID" class="text-right">{{ dtl.insuranceCoverageId }}</td>
          <td data-label="Insurance Coverage Name">{{ dtl.insuranceCoverageName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!insuranceCoverages.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0250A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.insuranceCoverageId" caption="Insurance Coverage ID" align="bottom"></sym-int>
        <sym-text v-model="detail.insuranceCoverageName" caption="Insurance Coverage Name" align="bottom"></sym-text>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add Insurance Coverage';
      }
      return 'Edit Insurance Coverage';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getInsuranceCoverageList()
        .then( list => {
          me.stopWait(wait);
          me.insuranceCoverages = list;
    ...
    getInsuranceCoverageList () {
      return get('api/insurance-coverages');
    },
    ...
    createRecord () {
      ...
      return ajax('api/insurance-coverages/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/insurance-coverages/' + this.detail.insuranceCoverageId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/insurance-coverages/' + detail.insuranceCoverageId + this.getDeleteQuery(detail.lockId), options);
    },
    ...
    onLoad () {
      ...
      dc.keyField = 'insuranceCoverageId';
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.insuranceCoverages.findIndex(obj => obj.insuranceCoverageId === success[0].insuranceCoverageId);
            if (index > -1) {
              me.insuranceCoverages[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.insuranceCoverages.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.insuranceCoverageId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('insuranceCoverageName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.insuranceCoverageId = dtl.insuranceCoverageId;
      d.insuranceCoverageName = dtl.insuranceCoverageName;
    ...
    onDeleteRecord (index) {
      const
        me = this,
        d = me.detail,
        dtl = me.insuranceCoverages[index];
      ...
      getSafeDeleteFlag('DbsInsuranceCoverage', me.detail.insuranceCoverageId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Insurance Coverage <b>' + me.insuranceCoverages[index].insuranceCoverageName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.insuranceCoverages.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.insuranceCoverages[me.recordIndex];

        dtl.insuranceCoverageId = d.insuranceCoverageId;
        dtl.insuranceCoverageName = d.insuranceCoverageName;
    ...
    clearRecordPad () {
      ...
      d.insuranceCoverageId = 0;
      d.insuranceCoverageName = '';
      d.sortSeq = 0;
      d.lockId = '';
    }

...
<sym-form id="dbs0280" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
  ...
  <div class="d-flex justify-center fixed-header" ref="list">
    <table class="striped-even mb-0 w-70">
      ...
      <tbody class="white">
        <tr v-for="(dtl, index) in courses" :key="index">
          <td data-label="Course ID" class="text-right">{{ dtl.courseId }}</td>
          <td data-label="Course Name">{{ dtl.courseName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!courses.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0280A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.courseId" caption="Course ID" align="bottom"></sym-int>
        <sym-text v-model="detail.courseName" caption="Course Name" align="bottom"></sym-text>
...
export default {
  ...
  methods: {
    ...
    createRecord () {
      ...
      return ajax('api/courses/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/courses/' + this.detail.courseId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/courses/' + detail.courseId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.courses.findIndex(obj => obj.courseId === success[0].courseId);
            if (index > -1) {
              me.courses[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.courses.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.courseId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('courseName');
              }, 100);
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsCourse', me.detail.courseId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Course <b>' + me.courses[index].courseName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.courses[me.recordIndex];

        dtl.courseId = d.courseId;

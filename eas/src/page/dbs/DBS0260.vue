  // Savings and Loans Setup

<template>
<section class="container w-40" :key="ts">
<sym-form id="dbs0260" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-100">
      <thead>
        <tr>
          ...
          <th class="w-55">Saving Loan Name</th>
          <th class="w-14 text-right">Sort Seq</th>
          <th class="w-21">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in savingLoans" :key="index">
          <td data-label="Saving Loan ID" class="text-right">{{ dtl.savingLoanId }}</td>
          <td data-label="Saving Loan Name">{{ dtl.savingLoanName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!savingLoans.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0260A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.savingLoanId" caption="Saving Loan ID" align="bottom"></sym-int>
        <sym-text v-model="detail.savingLoanName" caption="Saving Loan Name" align="bottom"></sym-text>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add Saving Loan';
      }
      return 'Edit Saving Loan';
    }
  },
  ...
  methods: {
    ...
    createRecord () {
      ...
      return ajax('api/savings-loans/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/savings-loans/' + this.detail.savingLoanId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/savings-loans/' + detail.savingLoanId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.savingLoans.findIndex(obj => obj.savingLoanId === success[0].savingLoanId);
            if (index > -1) {
              me.savingLoans[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.savingLoans.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.savingLoanId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('savingLoanName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.savingLoanId = dtl.savingLoanId;
      d.savingLoanName = dtl.savingLoanName;
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsSavingLoan', me.detail.savingLoanId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Saving Loan <b>' + me.savingLoans[index].savingLoanName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.savingLoans[me.recordIndex];

        dtl.savingLoanId = d.savingLoanId;
        dtl.savingLoanName = d.savingLoanName;

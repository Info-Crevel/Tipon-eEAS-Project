...
<template>
  ...
  <sym-form id="dbs0580" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 ">
      <thead>
        <tr>
          ...
          <th class="w-50">Request Type Name</th>
          <th class="w-20 text-right">Sort Seq</th>
          <th class="w-20">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in requestType" :key="index">
          <td class="text-right">{{ dtl.requestTypeId }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!requestType.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0580A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.requestTypeId" caption="RequestType ID" align="bottom"></sym-int>
        <sym-text v-model="detail.requestTypeName" caption="RequestType Name" align="bottom" @changing="onRequestTypeNameChanging"></sym-text>
...
export default {
  ...
  methods: {
    ...
    requestTypeNameCallback (e) {
      ...
      let index = me.requestType.findIndex(obj => obj.requestTypeName === e.proposedValue);
      if (index > -1) {
        e.message = 'Request Type <b>' + e.proposedValue + '</b> is already in the list.'
        return false;
      }
    ...
    createRecord () {
      ...
      return ajax('api/request-types/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/request-types/' + this.detail.requestTypeId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      const
        ...
        return ajax('api/request-types/' + detail.requestTypeId + this.getDeleteQuery(detail.lockId), options);
      },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.requestType.findIndex(obj => obj.requestTypeId === success[0].requestTypeId);
            if (index > -1) {
              me.requestType[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.requestType.push(success[0]); 
    ...
    onEditRecord (dtl, index) {
      ...
      d.requestTypeId = dtl.requestTypeId;
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsRequestType', me.detail.requestTypeId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Request Type <b>' + me.requestType[index].requestTypeName + '</b> will be deleted.<br><br>Continue?', { size: 'rg', scheme: 'warning' });
          } else {
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.requestType[me.recordIndex];

        dtl.requestTypeId = d.requestTypeId;

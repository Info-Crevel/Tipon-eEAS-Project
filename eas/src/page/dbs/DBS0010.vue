...
<sym-form id="dbs0010" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-50">
      <thead>
        <tr>
          ...
          <th class="w-50">Religion Name</th>
      ...
      <tbody class="white">
        <tr v-for="(dtl, index) in religions" :key="index">
          <td class="text-right">{{ dtl.religionId }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!religions.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0010A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.religionId" caption="Religion ID" align="bottom"></sym-int>
        <sym-text v-model="detail.religionName" caption="Religion Name" align="bottom"></sym-text>
...
export default {
  ...
  methods: {
    ...
    createRecord () {
      ...
      return ajax('api/religions/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/religions/' + this.detail.religionId + '/' + currentUserId, options);
    },
    ...
    return ajax('api/religions/' + detail.religionId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.religions.findIndex(obj => obj.religionId === success[0].religionId);
            if (index > -1) {
              me.religions[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.religions.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.religionId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('religionName');
              }, 100);
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsReligion', me.detail.religionId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Religion <b>' + me.religions[index].religionName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.religions[me.recordIndex];

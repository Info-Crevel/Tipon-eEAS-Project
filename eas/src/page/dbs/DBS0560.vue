// Member Suffix Definition

<template>
<section class="container p-0 w-40" :key="ts">
<sym-form id="dbs0560" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 ">
      <thead>
        <tr>
          ...
          <th class="w-30">Member Suffix Name</th>
          <th class="w-10 text-right">Sort Seq</th>
          <th class="w-10">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in memberSuffixes" :key="index">
          <td class="text-right">{{ dtl.memberSuffixId }}</td>
          <td>{{ dtl.memberSuffixName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!memberSuffixes.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0560A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.memberSuffixId" caption="MemberSuffix ID" align="bottom"></sym-int>
        <sym-text v-model="detail.memberSuffixName" caption="MemberSuffix Name" align="bottom" @changing="onMemberSuffixNameChanging"></sym-text>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add Member Suffix';
      }
      return 'Edit Member Suffix';
    }
  },  
  ...
  methods: {
    ...
    onMemberSuffixNameChanging (e) {
      e.message = 'Entry rejected.'
      e.callback = this.memberSuffixNameCallback;
    },

    memberSuffixNameCallback (e) {
      const me = this;

      let index = me.memberSuffixes.findIndex(obj => obj.memberSuffixName === e.proposedValue);
      if (index > -1) {
        e.message = 'Member Suffix <b>' + e.proposedValue + '</b> is already in the list.'
        return false;
      }
    ...
    loadData () {
      ...
      me.getMemberSuffixList()
        .then( list => {
          me.stopWait(wait);
          me.memberSuffixes = list;
    ...
    getMemberSuffixList () {
      return get('api/member-suffixes');
    },
    ...
    createRecord () {
      ...
      return ajax('api/member-suffixes/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/member-suffixes/' + this.detail.memberSuffixId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/member-suffixes/' + detail.memberSuffixId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.memberSuffixes.findIndex(obj => obj.memberSuffixId === success[0].memberSuffixId);
            if (index > -1) {
              me.memberSuffixes[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.memberSuffixes.push(success[0]); 
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.memberSuffixId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('memberSuffixName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.memberSuffixId = dtl.memberSuffixId;
      d.memberSuffixName = dtl.memberSuffixName;
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsMemberSuffix', me.detail.memberSuffixId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Member Suffix <b>' + me.memberSuffixes[index].memberSuffixName + '</b> will be deleted.<br><br>Continue?', { size: 'rg', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.memberSuffixes.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.memberSuffixes[me.recordIndex];

        dtl.memberSuffixId = d.memberSuffixId;
        dtl.memberSuffixName = d.memberSuffixName;

// Expense Charging Definition

<template>
<section class="container w-40" :key="ts">
<sym-form id="dbs0320" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-100">
      <thead>
        <tr>
          ...
          <th class="w-56">Expense Charging Name</th>
          <th class="w-14 text-right">Sort Seq</th>
          <th class="w-21">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in expenseChargings" :key="index">
          <td data-label="Expense Charging ID" class="text-right">{{ dtl.expenseChargingId }}</td>
          <td data-label="Expense Charging Name">{{ dtl.expenseChargingName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!expenseChargings.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0320A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.expenseChargingId" caption="Expense Charging ID" align="bottom"></sym-int>
        <sym-text v-model="detail.expenseChargingName" caption="Expense Charging Name" align="bottom"></sym-text>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add Expense Charging';
      }
      return 'Edit Expense Charging';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getExpenseChargingList()
        .then( list => {
          me.stopWait(wait);
          me.expenseChargings = list;
    ...
    getExpenseChargingList () {
      return get('api/expense-chargings');
    },
    ...
    createRecord () {
      ...
      return ajax('api/expense-chargings/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/expense-chargings/' + this.detail.expenseChargingId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/expense-chargings/' + detail.expenseChargingId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.expenseChargings.findIndex(obj => obj.expenseChargingId === success[0].expenseChargingId);
            if (index > -1) {
              me.expenseChargings[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.expenseChargings.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.expenseChargingId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('expenseChargingName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.expenseChargingId = dtl.expenseChargingId;
      d.expenseChargingName = dtl.expenseChargingName;
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsExpenseCharging', me.detail.expenseChargingId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Expense Charging <b>' + me.expenseChargings[index].expenseChargingName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.expenseChargings.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.expenseChargings[me.recordIndex];

        dtl.expenseChargingId = d.expenseChargingId;
        dtl.expenseChargingName = d.expenseChargingName;
    ...
    clearRecordPad () {
      ...
      d.expenseChargingId = 0;
      d.expenseChargingName = '';
      d.sortSeq = 0;
      d.lockId = '';
    }

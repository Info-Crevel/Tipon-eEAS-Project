// In-House Benefits Setup

<template>
<section class="container p-0" :key="ts">
<sym-form id="dbs0190" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-50">
      <thead>
        <tr>
          ...
          <th class="w-50">In-House Benefit Name</th>
          <th class="w-14 text-right">Sort Seq</th>
          <th class="w-26">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in inHouseBenefits" :key="index">
          <td data-label="In-House Benefit ID" class="text-right">{{ dtl.inHouseBenefitId }}</td>
          <td data-label="In-House Benefit Name">{{ dtl.inHouseBenefitName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!inHouseBenefits.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0190A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.inHouseBenefitId" caption="In-House Benefit ID" align="bottom"></sym-int>
        <sym-text v-model="detail.inHouseBenefitName" caption="In-House Benefit Name" align="bottom"></sym-text>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add In-House Benefit';
      }
      return 'Edit In-House Benefit';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getInHouseBenefitList()
        .then( list => {
          me.stopWait(wait);
          me.inHouseBenefits = list;
    ...
    getInHouseBenefitList () {
      return get('api/in-house-benefits');
    },
    ...
    createRecord () {
      ...
      return ajax('api/in-house-benefits/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/in-house-benefits/' + this.detail.inHouseBenefitId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/in-house-benefits/' + detail.inHouseBenefitId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.inHouseBenefits.findIndex(obj => obj.inHouseBenefitId === success[0].inHouseBenefitId);
            if (index > -1) {
              me.inHouseBenefits[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.inHouseBenefits.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.inHouseBenefitId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('inHouseBenefitName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.inHouseBenefitId = dtl.inHouseBenefitId;
      d.inHouseBenefitName = dtl.inHouseBenefitName;
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsInHouseBenefit', me.detail.inHouseBenefitId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('InHouse Benefit <b>' + me.inHouseBenefits[index].inHouseBenefitName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.inHouseBenefits.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.inHouseBenefits[me.recordIndex];

        dtl.inHouseBenefitId = d.inHouseBenefitId;
        dtl.inHouseBenefitName = d.inHouseBenefitName;
    ...
    clearRecordPad () {
      ...
      d.inHouseBenefitId = 0;
      d.inHouseBenefitName = '';
      d.sortSeq = 0;
      d.lockId = '';
    }

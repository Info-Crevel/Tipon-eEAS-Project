// School Definition Setup

<template>
<section class="container p-0" :key="ts">
<sym-form id="dbs0270" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-70">
      <thead>
        <tr>
          ...
          <th class="w-50">School Name</th>
          <th class="w-14 text-right">Sort Seq</th>
          <th class="w-26">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in schools" :key="index">
          <td data-label="School ID" class="text-right">{{ dtl.schoolId }}</td>
          <td data-label="School Name">{{ dtl.schoolName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!schools.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0270A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.schoolId" caption="School ID" align="bottom"></sym-int>
        <sym-text v-model="detail.schoolName" caption="School Name" align="bottom"></sym-text>
...
export default {
  ...
  methods: {
    ...
    createRecord () {
      ...
      return ajax('api/schools/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/schools/' + this.detail.schoolId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/schools/' + detail.schoolId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.schools.findIndex(obj => obj.schoolId === success[0].schoolId);
            if (index > -1) {
              me.schools[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.schools.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.schoolId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('schoolName');
              }, 100);
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsSchool', me.detail.schoolId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('School <b>' + me.schools[index].schoolName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.schools[me.recordIndex];

        dtl.schoolId = d.schoolId;

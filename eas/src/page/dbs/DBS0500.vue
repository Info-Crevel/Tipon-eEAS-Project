...
<sym-form id="dbs0500" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
  ...
  <div class="d-flex justify-center fixed-header" ref="list">
    <table class="striped-even mb-0 w-50">
      ...
      <tbody class="white">
        <tr v-for="(dtl, index) in skills" :key="index">
          <td data-label="Skill ID" class="text-right">{{ dtl.skillId }}</td>
          <td data-label="Skill Name">{{ dtl.skillName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!skills.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0500A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.skillId" caption="Skill ID" align="bottom"></sym-int>
...
export default {
  ...
  methods: {
    ...
    createRecord () {
      ...
      return ajax('api/skills/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/skills/' + this.detail.skillId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/skills/' + detail.skillId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.skills.findIndex(obj => obj.skillId === success[0].skillId);
            if (index > -1) {
              me.skills[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.skills.push(success[0]);
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsSkill', me.detail.skillId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Skill <b>' + me.skills[index].skillName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {

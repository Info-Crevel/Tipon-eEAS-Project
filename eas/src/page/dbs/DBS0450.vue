// Vaccine Type Definition

<template>
<section class="container p-0" :key="ts">
<sym-form id="dbs0450" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-50">
      <thead>
        <tr>
          ...
          <th class="w-50">Vaccine Type Name</th>
          <th class="w-14 text-right">Sort Seq</th>
          <th class="w-26">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in vaccineTypes" :key="index">
          <td data-label="Vaccine Type ID" class="text-right">{{ dtl.vaccineTypeId }}</td>
          <td data-label="Vaccine Type Name">{{ dtl.vaccineTypeName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!vaccineTypes.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0450A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.vaccineTypeId" caption="Vaccine Type ID" align="bottom"></sym-int>
        <sym-text v-model="detail.vaccineTypeName" caption="Vaccine Type Name" align="bottom" @changing="onVaccineTypeNameChanging"></sym-text>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add Vaccine Type';
      }
      return 'Edit Vaccine Type';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getVaccineTypeList()
        .then( list => {
          me.stopWait(wait);
          me.vaccineTypes = list;
    ...
    createRecord () {
      ...
      return ajax('api/vaccine-types/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/vaccine-types/' + this.detail.vaccineTypeId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/vaccine-types/' + detail.vaccineTypeId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.vaccineTypes.findIndex(obj => obj.vaccineTypeId === success[0].vaccineTypeId);
            if (index > -1) {
              me.vaccineTypes[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.vaccineTypes.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.vaccineTypeId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('vaccineTypeName');
              }, 100);
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsVaccineType', me.detail.vaccineTypeId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Vaccine Type <b>' + me.vaccineTypes[index].vaccineTypeName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.vaccineTypes.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.vaccineTypes[me.recordIndex];
    ...
    onVaccineTypeNameChanging (e) {
      e.message = 'Entry rejected.'
      e.callback = this.vaccineTypeNameCallback;
    },

    vaccineTypeNameCallback (e) {
      const me = this;

      let index = me.vaccineTypes.findIndex(obj => obj.vaccineTypeName === e.proposedValue);
      if (index > -1) {
        e.message = 'Vaccine Type Name <b>' + e.proposedValue + '</b> is already in the list.'
        return false;
      }

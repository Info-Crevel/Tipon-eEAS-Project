// Training Institution Setup

<template>
<section class="container p-0" :key="ts">
<sym-form id="dbs0220" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
  ...
  <div class="d-flex justify-center fixed-header" ref="list">
    <table class="striped-even mb-0 w-70">
      <thead>
        <tr>
          <th class="w-10 text-right">ID</th>
          <th class="w-50">Training Institution Name</th>
          <th class="w-14 text-right">Sort Seq</th>
          <th class="w-26">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in trainingInstitutions" :key="index">
          <td data-label="Training Institution ID" class="text-right">{{ dtl.trainingInstitutionId }}</td>
          <td data-label="Training Institution Name">{{ dtl.trainingInstitutionName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!trainingInstitutions.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0220A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.trainingInstitutionId" caption="Training Institution ID" align="bottom"></sym-int>
        <sym-text v-model="detail.trainingInstitutionName" caption="Training Institution Name" align="bottom"></sym-text>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add Training Institution';
      }
      return 'Edit Training Institution';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getTrainingInstitutionList()
        .then( list => {
          me.stopWait(wait);
          me.trainingInstitutions = list;
    ...
    getTrainingInstitutionList () {
      return get('api/training-institutions');
    },
    ...
    createRecord () {
      ...
      return ajax('api/training-institutions/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/training-institutions/' + this.detail.trainingInstitutionId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/training-institutions/' + detail.trainingInstitutionId + this.getDeleteQuery(detail.lockId), options);
    },
    ...
    onLoad () {
      ...
      dc.keyField = 'trainingInstitutionId';
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.trainingInstitutions.findIndex(obj => obj.trainingInstitutionId === success[0].trainingInstitutionId);
            if (index > -1) {
              me.trainingInstitutions[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.trainingInstitutions.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.trainingInstitutionId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('trainingInstitutionName');
              }, 100);
    ...
    onDeleteRecord (index) {
      const
        me = this,
        d = me.detail,
        dtl = me.trainingInstitutions[index];
      ...
      getSafeDeleteFlag('DbsTrainingInstitution', me.detail.trainingInstitutionId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Training Institution <b>' + me.trainingInstitutions[index].trainingInstitutionName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.trainingInstitutions.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.trainingInstitutions[me.recordIndex];

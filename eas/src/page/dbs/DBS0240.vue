...
<sym-form id="dbs0240" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
  ...
  <div class="d-flex justify-center fixed-header" ref="list">
    <table class="striped-even mb-0 w-90">
      <thead>
        <tr class="align-top">
          <th class="w-9 text-right">ID</th>
          <th class="w-50">Compliance Training Name</th>
      ...
      <tbody class="white">
        <tr v-for="(dtl, index) in complianceTrainings" :key="index">
          <td data-label="Compliance Training ID" class="text-right">{{ dtl.complianceTrainingId }}</td>
          <td data-label="Compliance Training Name">{{ dtl.complianceTrainingName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!complianceTrainings.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0240A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.complianceTrainingId" caption="Compliance Training ID" align="bottom"></sym-int>
        <sym-text v-model="detail.complianceTrainingName" caption="Compliance Training Name" align="bottom"></sym-text>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add Compliance Training';
      }
      return 'Edit Compliance Training';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getComplianceTrainingList()
        .then( list => {
          me.stopWait(wait);
          me.complianceTrainings = list;
    ...
    getComplianceTrainingList () {
      return get('api/compliance-trainings');
    },
    ...
    createRecord () {
      ...
      return ajax('api/compliance-trainings/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/compliance-trainings/' + this.detail.complianceTrainingId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/compliance-trainings/' + detail.complianceTrainingId + this.getDeleteQuery(detail.lockId), options);
    },
    ...
    onLoad () {
      ...
      dc.keyField = 'complianceTrainingId';
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.complianceTrainings.findIndex(obj => obj.complianceTrainingId === success[0].complianceTrainingId);
            if (index > -1) {
              me.complianceTrainings[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.complianceTrainings.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.complianceTrainingId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('complianceTrainingName');
              }, 100);
    ...
    onDeleteRecord (index) {
      const
        me = this,
        d = me.detail,
        dtl = me.complianceTrainings[index];
      ...
      getSafeDeleteFlag('DbsComplianceTraining', me.detail.complianceTrainingId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Compliance Training <b>' + me.complianceTrainings[index].complianceTrainingName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.complianceTrainings.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.complianceTrainings[me.recordIndex];
...
.record-editor-boxes {
  display: grid;
  grid-auto-flow: column;
  grid-template-columns: 5fr 14fr 1fr 3fr;
  gap: .5rem;
}

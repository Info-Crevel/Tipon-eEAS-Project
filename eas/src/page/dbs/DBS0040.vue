...
<sym-form id="dbs0040" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
  ...
  <div class="d-flex justify-center fixed-header" ref="list">
    <table class="striped-even mb-0 w-80">
      <thead>
        <tr class="align-top">
          <th class="w-6 text-right">ID</th>
          <th class="w-30">Relation Name</th>
          <th class="w-10 text-center">Upload</th>
          <th class="w-13 text-center">Occupation</th>
      ...
      <tbody class="white">
        <tr v-for="(dtl, index) in relations" :key="index">
          <td data-label="Relation ID" class="text-right">{{ dtl.relationId }}</td>
          <td data-label="Relation Name">{{ dtl.relationName }}</td>
          <td data-label="Upload" class="text-center">
            <i :class="getBooleanIconClass(dtl.uploadRequiredFlag)"></i>
          </td>
          <td data-label="Occupation" class="text-center">
            <i :class="getBooleanIconClass(dtl.occupationRequiredFlag)"></i>
          </td>
          <td data-label="Email" class="text-center">
            <i :class="getBooleanIconClass(dtl.emailRequiredFlag)"></i>
          </td>
          <td data-label="Sort Seq" class="text-right">{{ dtl.sortSeq }}</td>
          <!-- <td data-label="Action" class="p-1"> -->
          <td class="p-1">
  ...
  <div class="d-flex justify-center mt-3" v-if="!relations.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0040A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.relationId" caption="Relation ID" align="bottom"></sym-int>
        <sym-text v-model="detail.relationName" caption="Relation Name" align="bottom"></sym-text>
        ...
        <sym-check v-model="detail.occupationRequiredFlag" caption="Occupation" align="bottom" :caption-width="0" caption-align="center" check-align="center"></sym-check>
        <sym-check v-model="detail.emailRequiredFlag" caption="Email" align="bottom" :caption-width="0" caption-align="center" check-align="center"></sym-check>
...
export default {
  ...
  methods: {
    ...
    createRecord () {
      ...
      return ajax('api/relations/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/relations/' + this.detail.relationId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/relations/' + detail.relationId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.relations.findIndex(obj => obj.relationId === success[0].relationId);
            if (index > -1) {
              me.relations[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.relations.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.relationId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('relationName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.occupationRequiredFlag = dtl.occupationRequiredFlag;
      d.emailRequiredFlag = dtl.emailRequiredFlag;
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsRelation', me.detail.relationId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Relation <b>' + me.relations[index].relationName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.relations[me.recordIndex];
        ...
        dtl.occupationRequiredFlag = d.occupationRequiredFlag;
        dtl.emailRequiredFlag = d.emailRequiredFlag;
    ...
    clearRecordPad () {
      ...
      d.occupationRequiredFlag = false;
      d.emailRequiredFlag = false;
      d.sortSeq = 0;
      d.lockId = '';
    }
...
.record-editor-boxes {
  display: grid;
  grid-auto-flow: column;
  grid-template-columns: 5fr 12fr 1fr 1fr 1fr 4fr;
  gap: .5rem;
}
...
@media (max-width: 800px) {
  ...
  /* remove spacing effect of container */
  .container {
    padding: 0;
    margin: 0;
    max-width: 100%;

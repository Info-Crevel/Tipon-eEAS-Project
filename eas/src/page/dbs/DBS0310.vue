// Member Transaction Type Setup

<template>
<section class="container w-40" :key="ts">
<sym-form id="dbs0310" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-100">
      <thead>
        <tr>
          ...
          <th class="w-56">Member Transaction Type Name</th>
          <th class="w-14 text-right">Sort Seq</th>
          <th class="w-21">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in memberTransactionTypes" :key="index">
          <td data-label="Member Transaction Type ID" class="text-right">{{ dtl.memberTransactionTypeId }}</td>
          <td data-label="Member Transaction Type Name">{{ dtl.memberTransactionTypeName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!memberTransactionTypes.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0310A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.memberTransactionTypeId" caption="Transaction Type ID" align="bottom"></sym-int>
        <sym-text v-model="detail.memberTransactionTypeName" caption="Member Transaction Type Name" align="bottom"></sym-text>
...
export default {
  ...
  data () {
    return {
      memberTransactionTypes: [],

      detail: {
        memberTransactionTypeId: 0,
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add Member Transaction Type';
      }
      return 'Edit Member Transaction Type';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getMemberTransactionTypeList()
        .then( list => {
          me.stopWait(wait);
          me.memberTransactionTypes = list;
    ...
    getMemberTransactionTypeList () {
      return get('api/member-transaction-types');
    },
    ...
    createRecord () {
      ...
      return ajax('api/member-transaction-types/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/member-transaction-types/' + this.detail.memberTransactionTypeId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/member-transaction-types/' + detail.memberTransactionTypeId + this.getDeleteQuery(detail.lockId), options);
    },
    ...
    onLoad () {
      ...
      dc.keyField = 'memberTransactionTypeId';
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.memberTransactionTypes.findIndex(obj => obj.memberTransactionTypeId === success[0].memberTransactionTypeId);
            if (index > -1) {
              me.memberTransactionTypes[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.memberTransactionTypes.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.memberTransactionTypeId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('memberTransactionTypeName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.memberTransactionTypeId = dtl.memberTransactionTypeId;
      d.memberTransactionTypeName = dtl.memberTransactionTypeName;
    ...
    onDeleteRecord (index) {
      const
        me = this,
        d = me.detail,
        dtl = me.memberTransactionTypes[index];
      ...
      getSafeDeleteFlag('DbsMemberTransactionType', me.detail.memberTransactionTypeId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Member Transaction Type <b>' + me.memberTransactionTypes[index].memberTransactionTypeName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.memberTransactionTypes.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.memberTransactionTypes[me.recordIndex];

        dtl.memberTransactionTypeId = d.memberTransactionTypeId;
        dtl.memberTransactionTypeName = d.memberTransactionTypeName;
    ...
    clearRecordPad () {
      ...
      d.memberTransactionTypeId = 0;
      d.memberTransactionTypeName = '';
      d.sortSeq = 0;
      d.lockId = '';
    }

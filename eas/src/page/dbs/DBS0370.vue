...
<sym-form id="dbs0370" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
  ...
  <div class="d-flex justify-center fixed-header" ref="list">
    <table class="striped-even mb-0 w-55">
      <thead>
        <tr class="align-top">
          <th class="w-10 text-right">ID</th>
          <th class="w-42">CDA Training Name</th>
          <th class="w-10 text-center">Upload</th>
          <th class="w-12 text-right">Sort Seq</th>
          <th class="w-26">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in cdaTrainings" :key="index">
          <td data-label="CDA Training ID" class="text-right">{{ dtl.cdaTrainingId }}</td>
          <td data-label="CDA Training Name">{{ dtl.cdaTrainingName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!cdaTrainings.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0370A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.cdaTrainingId" caption="CDA Training ID" align="bottom"></sym-int>
        <sym-text v-model="detail.cdaTrainingName" caption="CDA Training Name" align="bottom"></sym-text>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add CDA Training';
      }
      return 'Edit CDA Training';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getCdaTrainingList()
        .then( list => {
          me.stopWait(wait);
          me.cdaTrainings = list;
    ...
    createRecord () {
      ...
      return ajax('api/cda-trainings/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/cda-trainings/' + this.detail.cdaTrainingId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/cda-trainings/' + detail.cdaTrainingId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.cdaTrainings.findIndex(obj => obj.cdaTrainingId === success[0].cdaTrainingId);
            if (index > -1) {
              me.cdaTrainings[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.cdaTrainings.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.cdaTrainingId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('cdaTrainingName');
              }, 100);
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsCdaTraining', me.detail.cdaTrainingId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('CDA Training <b>' + me.cdaTrainings[index].cdaTrainingName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.cdaTrainings[me.recordIndex];
...
.record-editor-boxes {
  display: grid;
  grid-auto-flow: column;
  grid-template-columns: 4fr 9fr 1fr 3fr;
  gap: .5rem;
}

// Non-Revenue Qualification Setup

<template>
<section class="container p-0" :key="ts">
<sym-form id="dbs0440" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-50">
      <thead>
        <tr>
          ...
          <th class="w-50">Non-Revenue Qualification Name</th>
          <th class="w-14 text-right">Sort Seq</th>
          <th class="w-26">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in nonRevenueQualifications" :key="index">
          <td data-label="Non-Revenue Qualification ID" class="text-right">{{ dtl.nonRevenueQualificationId }}</td>
          <td data-label="Non-Revenue Qualification Name">{{ dtl.nonRevenueQualificationName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!nonRevenueQualifications.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0440A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.nonRevenueQualificationId" caption="Non-Rev Qualification ID" align="bottom"></sym-int>
        <sym-text v-model="detail.nonRevenueQualificationName" caption="Non-Revenue Qualification Name" align="bottom"></sym-text>
...
export default {
  ...
  data () {
    return {
      nonRevenueQualifications: [],

      detail: {
        nonRevenueQualificationId: 0,
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add Non-Revenue Qualification';
      }
      return 'Edit Non-Revenue Qualification';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getNonRevenueQualificationList()
        .then( list => {
          me.stopWait(wait);
          me.nonRevenueQualifications = list;
    ...
    getNonRevenueQualificationList () {
      return get('api/non-revenue-qualifications');
    },
    ...
    createRecord () {
      ...
      return ajax('api/non-revenue-qualifications/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/non-revenue-qualifications/' + this.detail.nonRevenueQualificationId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/non-revenue-qualifications/' + detail.nonRevenueQualificationId + this.getDeleteQuery(detail.lockId), options);
    },
    ...
    onLoad () {
      ...
      dc.keyField = 'nonRevenueQualificationId';
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.nonRevenueQualifications.findIndex(obj => obj.nonRevenueQualificationId === success[0].nonRevenueQualificationId);
            if (index > -1) {
              me.nonRevenueQualifications[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.nonRevenueQualifications.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.nonRevenueQualificationId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('nonRevenueQualificationName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.nonRevenueQualificationId = dtl.nonRevenueQualificationId;
      d.nonRevenueQualificationName = dtl.nonRevenueQualificationName;
    ...
    onDeleteRecord (index) {
      const
        me = this,
        d = me.detail,
        dtl = me.nonRevenueQualifications[index];
      ...
      getSafeDeleteFlag('DbsNonRevenueQualification', me.detail.nonRevenueQualificationId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Non-Revenue Qualification <b>' + me.nonRevenueQualifications[index].nonRevenueQualificationName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.nonRevenueQualifications.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.nonRevenueQualifications[me.recordIndex];

        dtl.nonRevenueQualificationId = d.nonRevenueQualificationId;
        dtl.nonRevenueQualificationName = d.nonRevenueQualificationName;
    ...
    clearRecordPad () {
      ...
      d.nonRevenueQualificationId = 0;
      d.nonRevenueQualificationName = '';
      d.sortSeq = 0;
      d.lockId = '';
    }

...
<sym-form id="dbs0230" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-70">
      <thead>
        <tr>
          ...
          <th class="w-50">Assessment Center Name</th>
          <th class="w-14 text-right">Sort Seq</th>
          <th class="w-26">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in assessmentCenters" :key="index">
          <td data-label="Assessment Center ID" class="text-right">{{ dtl.assessmentCenterId }}</td>
          <td data-label="Assessment Center Name">{{ dtl.assessmentCenterName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!assessmentCenters.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0230A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.assessmentCenterId" caption="Assessment Center ID" align="bottom"></sym-int>
        <sym-text v-model="detail.assessmentCenterName" caption="Assessment Center Name" align="bottom"></sym-text>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add Assessment Center';
      }
      return 'Edit Assessment Center';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getAssessmentCenterList()
        .then( list => {
          me.stopWait(wait);
          me.assessmentCenters = list;
    ...
    getAssessmentCenterList () {
      return get('api/assessment-centers');
    },
    ...
    createRecord () {
      ...
      return ajax('api/assessment-centers/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/assessment-centers/' + this.detail.assessmentCenterId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/assessment-centers/' + detail.assessmentCenterId + this.getDeleteQuery(detail.lockId), options);
    },
    ...
    onLoad () {
      ...
      dc.keyField = 'assessmentCenterId';
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.assessmentCenters.findIndex(obj => obj.assessmentCenterId === success[0].assessmentCenterId);
            if (index > -1) {
              me.assessmentCenters[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.assessmentCenters.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.assessmentCenterId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('assessmentCenterName');
              }, 100);
    ...
    onDeleteRecord (index) {
      const
        me = this,
        d = me.detail,
        dtl = me.assessmentCenters[index];
      ...
      getSafeDeleteFlag('DbsAssessmentCenter', me.detail.assessmentCenterId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Assessment Center <b>' + me.assessmentCenters[index].assessmentCenterName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.assessmentCenters.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.assessmentCenters[me.recordIndex];

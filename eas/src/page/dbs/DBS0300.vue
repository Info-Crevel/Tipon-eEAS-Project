...
<sym-form id="dbs0300" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-100">
      <thead>
        <tr>
          ...
          <th class="w-56">Bank Location Name</th>
          <th class="w-14 text-right">Sort Seq</th>
          <th class="w-21">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in bankLocations" :key="index">
          <td data-label="Bank Location ID" class="text-right">{{ dtl.bankLocationId }}</td>
          <td data-label="Bank Location Name">{{ dtl.bankLocationName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!bankLocations.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0300A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.bankLocationId" caption="Bank Location ID" align="bottom"></sym-int>
        <sym-text v-model="detail.bankLocationName" caption="Bank Location Name" align="bottom"></sym-text>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add Bank Location';
      }
      return 'Edit Bank Location';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getBankLocationList()
        .then( list => {
          me.stopWait(wait);
          me.bankLocations = list;
    ...
    createRecord () {
      ...
      return ajax('api/bank-locations/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/bank-locations/' + this.detail.bankLocationId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/bank-locations/' + detail.bankLocationId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.bankLocations.findIndex(obj => obj.bankLocationId === success[0].bankLocationId);
            if (index > -1) {
              me.bankLocations[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.bankLocations.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.bankLocationId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('bankLocationName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.bankLocationId = dtl.bankLocationId;
      d.bankLocationName = dtl.bankLocationName;
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsBankLocation', me.detail.bankLocationId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Bank Location <b>' + me.bankLocations[index].bankLocationName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.bankLocations.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.bankLocations[me.recordIndex];

        dtl.bankLocationId = d.bankLocationId;
        dtl.bankLocationName = d.bankLocationName;

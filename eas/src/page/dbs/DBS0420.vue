...
<sym-form id="dbs0420" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-50">
      <thead>
        <tr>
          ...
          <th class="w-50">RNR Recording Name</th>
      ...
      <tbody class="white">
        <tr v-for="(dtl, index) in rnrRecordings" :key="index">
          <td data-label="RNR Recording Type ID" class="text-right">{{ dtl.rnrRecordingId }}</td>
          <td data-label="RNR Recording Type Name">{{ dtl.rnrRecordingName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!rnrRecordings.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0420A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.rnrRecordingId" caption="RNR Recording ID" align="bottom"></sym-int>
        <sym-text v-model="detail.rnrRecordingName" caption="RNR Recording Name" align="bottom"></sym-text>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add RNR Recording';
      }
      return 'Edit RNR Recording';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getRnrRecordingList()
        .then( list => {
          me.stopWait(wait);
          me.rnrRecordings = list;
    ...
    createRecord () {
      ...
      return ajax('api/rnr-recordings/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/rnr-recordings/' + this.detail.rnrRecordingId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/rnr-recordings/' + detail.rnrRecordingId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.rnrRecordings.findIndex(obj => obj.rnrRecordingId === success[0].rnrRecordingId);
            if (index > -1) {
              me.rnrRecordings[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.rnrRecordings.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.rnrRecordingId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('rnrRecordingName');
              }, 100);
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('DbsRnrRecording', me.detail.rnrRecordingId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('RNR Recording <b>' + me.rnrRecordings[index].rnrRecordingName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.rnrRecordings[me.recordIndex];

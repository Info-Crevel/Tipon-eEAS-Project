// NCII Qualification Title Setup

<template>
<section class="container p-0" :key="ts">
<sym-form id="dbs0210" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 w-65">
      <thead>
        <tr class="align-top">
          ...
          <th class="w-50">NCII Title Name</th>
      ...
      <tbody class="white">
        <tr v-for="(dtl, index) in nciiQualificationTitles" :key="index">
          <td data-label="NCII Title ID" class="text-right">{{ dtl.nciiQualificationTitleId }}</td>
          <td data-label="NCII Title Name">{{ dtl.nciiQualificationTitleName }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!nciiQualificationTitles.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="dbs0210A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.nciiQualificationTitleId" caption="NCII Title ID" align="bottom"></sym-int>
        <sym-text v-model="detail.nciiQualificationTitleName" caption="NCII Title Name" align="bottom"></sym-text>
...
export default {
  ...
  computed: {
    editorTitle () {
      if (this.isAdding) {
        return 'Add NCII Qualification Title';
      }
      return 'Edit NCII Qualification Title';
    }
  },
  ...
  methods: {
    ...
    loadData () {
      ...
      me.getNCIIQualificationTitleList()
        .then( list => {
          me.stopWait(wait);
          me.nciiQualificationTitles = list;
    ...
    getNCIIQualificationTitleList () {
      return get('api/ncii-qualification-titles');
    },
    ...
    createRecord () {
      ...
      return ajax('api/ncii-qualification-titles/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/ncii-qualification-titles/' + this.detail.nciiQualificationTitleId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/ncii-qualification-titles/' + detail.nciiQualificationTitleId + this.getDeleteQuery(detail.lockId), options);
    },
    ...
    onLoad () {
      ...
      dc.keyField = 'nciiQualificationTitleId';
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.nciiQualificationTitles.findIndex(obj => obj.nciiQualificationTitleId === success[0].nciiQualificationTitleId);
            if (index > -1) {
              me.nciiQualificationTitles[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.nciiQualificationTitles.push(success[0]);
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.nciiQualificationTitleId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('nciiQualificationTitleName');
              }, 100);
    ...
    onDeleteRecord (index) {
      const
        me = this,
        d = me.detail,
        dtl = me.nciiQualificationTitles[index];
      ...
      getSafeDeleteFlag('DbsNCIIQualificationTitle', me.detail.nciiQualificationTitleId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('NCII Qualification Title <b>' + me.nciiQualificationTitles[index].nciiQualificationTitleName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.nciiQualificationTitles.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.nciiQualificationTitles[me.recordIndex];
...
.record-editor-boxes {
  display: grid;
  grid-auto-flow: column;
  grid-template-columns: 3fr 11fr 1fr 3fr;
  gap: .5rem;
}

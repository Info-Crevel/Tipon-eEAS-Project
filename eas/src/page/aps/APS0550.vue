...
<template>
  ...
  <sym-form id="aps0550" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
  ...
  <div class="d-flex justify-center fixed-header" ref="list">
    <table class="striped-even mb-0 ">
      ...
      <tbody class="white">
        <tr v-for="(dtl, index) in approvers" :key="index">
          <td class="text-right">{{ dtl.approverId }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!approvers.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="aps0550A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.approverId" caption="Approver ID" align="bottom"></sym-int>
        <sym-text v-model="detail.memberEmployeeId" caption="Employee ID" align="bottom" @changing="onMemberEmployeeIdChanging"></sym-text>
        <sym-text v-model="detail.memberName" caption="Approver Name" align="bottom"></sym-text>
  ...
  methods: {
    ...
    userIdCallback (e) {
      ...
      return getList('dbo.SecUser', 'UserId, UserName', '', filter).then(
        user => {
          if (user && user.length) {        
            let index = me.approvers.findIndex(obj => obj.userId === e.proposedValue);
            if (index > -1) {
    ...
    onUserIdSearchResult (result) {
      ...
      const
        data = result[0],
        detail = this.detail;
        let index = this.approvers.findIndex(obj => obj.userId === data.userId);
        if (index > -1) {
      ...
      return getList('dbo.HrsMember', 'MemberEmployeeId, MemberName, Email', '', filter).then(
        member => {
          if (member && member.length) {        
            let index = me.approvers.findIndex(obj => obj.memberEmployeeId === e.proposedValue);
            if (index > -1) {
    ...
    createRecord () {
      ...
      return ajax('api/approvers/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/approvers/' + this.detail.approverId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/approvers/' + detail.approverId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.approvers.findIndex(obj => obj.approverId === success[0].approverId);
            if (index > -1) {
              me.approvers[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.approvers.push(success[0]); 
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('ApsApprover', me.detail.approverId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Approver <b>' + me.approvers[index].approverName + '</b> will be deleted.<br><br>Continue?', { size: 'rg', scheme: 'warning' });
          } else {
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.approvers[me.recordIndex];

        dtl.approverId = d.approverId;

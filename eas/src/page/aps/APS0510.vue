// Payable Request Type

<template>
  <section class="container p-0   w-70" :key="ts">
  <sym-form id="aps0510" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 ">
      <thead>
        <tr>
          ...
          <th class="w-70">Request Type Name</th>
          <th class="w-10">Sort Seq</th>
          <th class="w-10">Action</th>
        </tr>
      </thead>
      <tbody class="white">
        <tr v-for="(dtl, index) in requestTypes" :key="index">
          <td class="text-right">{{ dtl.apsRequestTypeId }}</td>
          <td>{{ dtl.requestTypeName }}</td>
          <td>{{ dtl.sortSeq}}</td>
          <td class="p-1">
  ...
  <div class="d-flex justify-center mt-3" v-if="!requestTypes.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="aps0510A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.apsRequestTypeId" caption="ID" align="bottom"></sym-int>
        <sym-text v-model="detail.requestTypeName" caption="Request Type Name" align="bottom" @changing="onRequestTypeNameChanging"></sym-text>
...
export default {
  ...
  methods: {
    ...
    onRequestTypeNameChanging (e) {
      ...
      e.callback = this.requestTypeNameCallback;
    },


    requestTypeNameCallback (e) {
      const me = this;
      let filter = "RequestTypeName='" + e.proposedValue +"'";
      return getList('dbo.ApsRequestType', 'ApsRequestTypeId, RequestTypeName', '', filter).then(
        requestType => {
          if (requestType && requestType.length) {        
            let index = me.requestTypes.findIndex(obj => obj.requestTypeName === e.proposedValue);
            if (index > -1) {
    ...
    getRequestTypeList () {
      return get('api/payable-request-types');
    },
    ...
    createRecord () {
      ...
      return ajax('api/payable-request-types/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/payable-request-types/' + this.detail.apsRequestTypeId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/payable-request-types/' + detail.apsRequestTypeId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.requestTypes.findIndex(obj => obj.apsRequestTypeId === success[0].apsRequestTypeId);
            if (index > -1) {
              me.requestTypes[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.requestTypes.push(success[0]); 
            ...
            if (me.isAdding) {
              ...
              me.clearRecordPad();
              me.detail.apsRequestTypeId = -1;
              me.refreshOldRefs();

              setTimeout(() => {
                me.scrollToBottom('list');
                this.setFocus('requestTypeName');
              }, 100);
    ...
    onEditRecord (dtl, index) {
      ...
      d.apsRequestTypeId = dtl.apsRequestTypeId;
      d.requestTypeName = dtl.requestTypeName;
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('ApsRequestType', me.detail.apsRequestTypeId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Request Type <b>' + me.requestTypes[index].requestTypeName + '</b> will be deleted.<br><br>Continue?', { size: 'rg', scheme: 'warning' });
          } else {
        ...
        .then( success => {
          if (success) {
            me.requestTypes.splice(index, 1);
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.requestTypes[me.recordIndex];

        dtl.apsRequestTypeId = d.apsRequestTypeId;
        dtl.requestTypeName = d.requestTypeName;

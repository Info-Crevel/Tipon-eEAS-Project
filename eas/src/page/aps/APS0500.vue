...
<template>
  ...
  <sym-form id="aps0500" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
  ...
  <div class="d-flex justify-center fixed-header" ref="list">
    <table class="striped-even mb-0 ">
      ...
      <tbody class="white">
        <tr v-for="(dtl, index) in reviewers" :key="index">
          <td class="text-right">{{ dtl.reviewerId }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!reviewers.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="aps0500A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.reviewerId" caption="Reviewer ID" align="bottom"></sym-int>
        <sym-text v-model="detail.memberEmployeeId" caption="Employee ID" align="bottom" @changing="onMemberEmployeeIdChanging"></sym-text>
        <sym-text v-model="detail.memberName" caption="Reviewer Name" align="bottom"></sym-text>
        <sym-text v-model="detail.email" caption="Email" align="bottom"></sym-text>
  ...
  methods: {
    ...
    userIdCallback (e) {
      ...
      return getList('dbo.SecUser', 'UserId, UserName', '', filter).then(
        user => {
          if (user && user.length) {        
            let index = me.reviewers.findIndex(obj => obj.userId === e.proposedValue);
            if (index > -1) {
    ...
    onUserIdSearchResult (result) {
      ...
      const
        data = result[0],
        detail = this.detail;
        let index = this.reviewers.findIndex(obj => obj.userId === data.userId);
        if (index > -1) {
    ...
    memberEmployeeIdCallback (e) {
      ...
      return getList('dbo.HrsMember', 'MemberEmployeeId, MemberName, Email', '', filter).then(
        member => {
          if (member && member.length) {        
            let index = me.reviewers.findIndex(obj => obj.memberEmployeeId === e.proposedValue);
            if (index > -1) {
              e.message = 'Employee ID <b>' + e.proposedValue + '</b> is already in the list.'
              return false;
            }

            me.detail.memberName = member[0].memberName;
            me.detail.email = member[0].email;
            return true;
          }
          return false;
        },
    ...
    createRecord () {
      ...
      return ajax('api/reviewers/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/reviewers/' + this.detail.reviewerId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/reviewers/' + detail.reviewerId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.reviewers.findIndex(obj => obj.reviewerId === success[0].reviewerId);
            if (index > -1) {
              me.reviewers[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.reviewers.push(success[0]); 
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('ApsReviewer', me.detail.reviewerId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Reviewer <b>' + me.reviewers[index].reviewerName + '</b> will be deleted.<br><br>Continue?', { size: 'rg', scheme: 'warning' });
          } else {
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.reviewers[me.recordIndex];

        dtl.reviewerId = d.reviewerId;

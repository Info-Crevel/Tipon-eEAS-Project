...
<section class="container p-0" :key="ts">
   <sym-form id="aps0100" :caption="pageName" cardClass="curved-0" headerClass="app-form-header" footerClass="border-top-main frs-form-footer darken-2 py-0" bodyClass="app-form-body pb-3 ">
  <div slot="toolbar" class="action-buttons p-1 px-3 border-bottom-main">
  ...
  <div>
    ...
    <div class="d-flex gap-1">
      ...
      <!--DO NOT DELETE! Disable Tax field as per meeting 083125--> 
      ...
      <sym-combo v-model="trx.payableTaxCode" :caption-width="42" :input-width="90" caption="Tax" display-field="payableTaxName" :datasource="taxes" @changed="onPayableTaxCodeChanged"></sym-combo>
        <sym-dec class="text-right"  v-model="trx.taxPercentage" :caption-width="30" :inpu>t-width="20" caption="Percentage" @changed="onAmountChanged"></sym-dec> 
...
import APS0100_09052025 from './APS0100_09052025.vue';
...
export default {
  ...
  methods: {
    ...
    clusterIdCallback (e) {
      const me = this;
      let filter = "ClusterId='" + e.proposedValue ;
  ...
  console.log('me.trx.trxId',this.trx.trxId)
  if (this.trx.trxId == -1) {
  ...
  if (this.trxDetails.length==2) {

this.trxDetails = this.trxDetails.map(item => ({
  ...item,
  debitAmount: item.debitAmount > 0 ? this.trx.amount : item.debitAmount,
  creditAmount: item.creditAmount > 0 ? this.trx.amount : item.creditAmount
}));

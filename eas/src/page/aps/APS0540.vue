// Request Doc Type Setup

<template>
<section class="container p-0 w-60" :key="ts">
<sym-form id="aps0540" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
  ...
  <div class="d-flex justify-center fixed-header" ref="list">
    <table class="striped-even mb-0 ">
      ...
      <tbody class="white">
        <tr v-for="(dtl, index) in apsDocTypes" :key="index">
          <td class="text-right">{{ dtl.apsDocTypeId }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!apsDocTypes.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="aps0540A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="record-editor-boxes">
        <sym-int v-model="detail.apsDocTypeId" caption="Doc Type ID" align="bottom"></sym-int>
...
export default {
  ...
  methods: {
    ...
    createRecord () {
      ...
      return ajax('api/aps-doc-types/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/aps-doc-types/' + this.detail.apsDocTypeId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/aps-doc-types/' + detail.apsDocTypeId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.apsDocTypes.findIndex(obj => obj.apsDocTypeId === success[0].apsDocTypeId);
            if (index > -1) {
              me.apsDocTypes[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.apsDocTypes.push(success[0]);
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('ApsDocType', me.detail.apsDocTypeId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Document Type <b>' + me.apsDocTypes[index].apsDocTypeName + '</b> will be deleted.<br><br>Continue?', { size: 'md', scheme: 'warning' });
          } else {
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.apsDocTypes[me.recordIndex];

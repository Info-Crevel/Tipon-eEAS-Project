...
<section class="container p-0" :key="ts">
   <sym-form id="fin0040" :caption="pageName" cardClass="curved-0" headerClass="app-form-header" footerClass="border-top-main app-form-footer darken-2 py-0" bodyClass="app-form-body pb-3 ">
  ...
  <div>
    ...
    <div class="d-flex">
      <div class="d-flex justify-between align-items-end fw-118 ">
        <sym-int v-model="trx.trxId" :caption-width="42" :input-width="34" caption="Transaction ID" lookupId="FinTrxJournalEntry" @lostfocus="onTrxIdLostFocus" @changing="onTrxIdChanging" @changed="onTrxIdChanged" @searchresult="onTrxIdSearchResult"></sym-int>
    ...
    <div class="grid cols-5 gap-1">
      <sym-combo v-model="trx.trxTypeId" :caption-width="42" :input-width="100" caption="Transaction Type" display-field="trxTypeName" :datasource="trxTypes" ></sym-combo>

      <sym-text v-model="trx.reference" :caption-width="42" :input-width="50" caption="Reference" ></sym-text>
    </div>
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="fin0040A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="fin0040B" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
...
<div class="to-print">
   <span class="lg-1 app-bold"> Voucher No: {{ trx.documentId }}</span>
  ...
  <table class="mt-2 white text-charcoal mb-0">
    <tbody>
      ...
      <tr class="doc-header">
        <td colspan="3" class="text-center display-9 app-bold ">JOURNAL ENTRY VOUCHER</td>
        <td colspan="1">No. <span class="lg-1 app-bold">{{ trx.trxId }}</span></td>
      </tr>
      <tr class="doc-header">
...
export default {
  ...
  methods: {
    ...
    // onTrxTypeIdChanged(){
    //   this.trxTypes.forEach((trxType) => {
    //       if (trxType.trxTypeId == this.trx.trxTypeId) {
    //         this.trx.trx = tax.percentage;
    ...
    getTrx () {
      return get('api/journal-entry/' + this.trx.trxId);
    },
    ...
    getReferences () {
      ...
      return get('api/references/fin0040');
    },

    getChangeLog () {
      return get('api/journal-entry/' + this.trx.trxId + '/log');
    },
    ...
    createRecord () {
      ...
      return ajax('api/journal-entry/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/journal-entry/' + this.trx.trxId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/journal-entry/' + trx.trxId + '/' + trx.lockId, options);
    },

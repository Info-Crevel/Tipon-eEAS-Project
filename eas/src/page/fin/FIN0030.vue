...
<template>
  ...
  <sym-form id="fin0030" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
    ...
    <table class="striped-even mb-0 ">
      <thead>
        <tr>
          ...
          <th class="w-30">Bank Name</th>
          <th class="w-10">Next Check #</th>
      ...
      <tbody class="white">
        <tr v-for="(dtl, index) in banks" :key="index">
          <td class="text-right">{{ dtl.bankId }}</td>
          <td>{{ dtl.bankName }}</td>
          <td>{{ dtl.bankNextSeqId }}</td>
          <td>{{ dtl.accountId }}</td>
  ...
  <div class="d-flex justify-center mt-3" v-if="!banks.length">
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="fin0030A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="scroller-container">
        <div class="record-editor-boxes">
          <sym-int v-model="detail.bankId" caption="ID" align="bottom" :caption-width="100"></sym-int>
          <sym-text v-model="detail.bankName" caption="Bank Name" align="bottom" @changing="onBankNameChanging"></sym-text>
          <sym-int v-model="detail.bankNextSeqId" caption="Next Check #" align="bottom"></sym-int>
...
export default {
  ...
  methods: {
    ...
    onBankNameChanging (e) {
      e.message = 'Entry rejected.'
      e.callback = this.bankNameCallback;
    },

    bankNameCallback (e) {
      const me = this;

      let index = me.banks.findIndex(obj => obj.bankName === e.proposedValue);
      if (index > -1) {
        e.message = 'Bank Name <b>' + e.proposedValue + '</b> is already in the list.'
        return false;
      }
    ...
    createRecord () {
      ...
      return ajax('api/acct-banks/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/acct-banks/' + this.detail.bankId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      const
        ...
        return ajax('api/acct-banks/' + detail.bankId + this.getDeleteQuery(detail.lockId), options);
    },
      ...
      promise.then(
        (success) => {
          ...
          if (success && success.length) {
            let index = me.banks.findIndex(obj => obj.bankId === success[0].bankId);
            if (index > -1) {
              me.banks[index].lockId = success[0].lockId;
            } else {
              if (me.isAdding) {
                me.banks.push(success[0]); 
    ...
    onEditRecord (dtl, index) {
      ...
      d.bankId = dtl.bankId;
      d.bankName = dtl.bankName;
      d.bankNextSeqId = dtl.bankNextSeqId;
    ...
    onDeleteRecord (index) {
      ...
      getSafeDeleteFlag('FinBank', me.detail.bankId)
        .then( safe => {
          if (safe) {
            return me.dialog.confirm('Bank <b>' + me.banks[index].bankName + '</b> will be deleted.<br><br>Continue?', { size: 'rg', scheme: 'warning' });
          } else {
    ...
    onSubmitDetail () {
      ...
      if (me.isAdding) {
        ...
        dtl = me.banks[me.recordIndex];

        dtl.bankId = d.bankId;
        dtl.bankName = d.bankName;
        dtl.bankNextSeqId = d.bankNextSeqId;
...
.record-editor-boxes {
  display: grid;
  grid-auto-flow: column;
  grid-template-columns: .5fr 1fr .5fr .4fr 1.5fr ;
  gap: .5rem;
  /* width: 100vw; */
}

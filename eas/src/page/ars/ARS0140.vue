...
<sym-form id="ars0140" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
  ...
  <div class="d-flex justify-between align-items-end fw-96">
    <sym-int v-model="deminimis.memberDeminimisId" :caption-width="35" :input-width="20" caption="Deminimis ID" lookupId="ArsMemberDeminimis" @lostfocus="onMemberDeminimisIdLostFocus" @changing="onMemberDeminimisIdChanging" @changed="onMemberDeminimisIdChanged" @searchfill="onMemberDeminimisIdSearchFill" @searchresult="onMemberDeminimisIdSearchResult"></sym-int>
  ...
  <div class="box-1 app-box-style gap">
    <sym-int v-model="deminimis.memberId" align="bottom" :caption-width="50" caption="Member ID " display-field="memberName" lookupId="HrsMemberDeminimis" @changing="onMemberIdChanging" @searchresult="onMemberIdSearchResult"></sym-int> 
    <sym-text v-model="deminimis.memberName" align="bottom" :caption-width="60" caption="Member Name" ></sym-text>
    <sym-int v-model="deminimis.memberRequestId" align="bottom" :caption-width="34" caption="MRF #" display-field="memberRequestName" lookupId="ArsMemberRequestDeduction" @changing="onMemberRequestIdChanging" @searchfill="onMemberRequestIdSearchFill" @searchresult="onMemberRequestIdSearchResult"></sym-int> 
    <sym-text v-model="deminimis.memberRequestName" align="bottom" :caption-width="10" caption="Member Request Name" ></sym-text>
    <sym-memo v-model="deminimis.remarks" :caption-width="35"  caption="Remarks" align="bottom"></sym-memo>
  </div>
...
<div class="board p-1 mb-0 w-100">
  <form id="ars0140A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
    <div class="trx-editor-boxes gap">
      <sym-combo v-model="trx.payTrxCode" caption="Deminimis" align="bottom" display-field="payTrxName" :datasource="payTrx" @changing="onPayTrxCodeChanging"></sym-combo>
...
export default {
  ...
  methods: {
    ...
    getTargetPath() {
      ...
      if (me.deminimis.memberRequestId) {
        q.memberRequestId = me.deminimis.memberRequestId;
      }
      
      if (me.deminimis.memberId) {
        q.memberId = me.deminimis.memberId;
      }

      if (me.deminimis.memberName) {
        q.memberName = me.deminimis.memberName;
      }

      if (me.deminimis.memberRequestName) {
        q.memberRequestName = me.deminimis.memberRequestName;
      }
    ...
    syncValues(p, q) {
      ...
      if ("memberRequestId" in q && me.core.isInteger(q.memberRequestId)) {
        ...
        me.deminimis.memberRequestId = parseInt(q.memberRequestId);
    
      }

      if ("memberId" in q && me.core.isInteger(q.memberId)) {
       
        me.deminimis.memberId = parseInt(q.memberId);
      ...
      if ("memberName" in q && me.core.isString(q.memberName)) {
        ...
        me.deminimis.memberName = q.memberName;
    
      }

      if ("memberRequestName" in q && me.core.isString(q.memberRequestName)) {
       
        me.deminimis.memberRequestName = q.memberRequestName;
    ...
    payTrxCodeCallback(e) {
      ...
      if (index > -1) {
        ...
        e.message = 'Deminimis <b>' + payTrxName + '</b> is already in the list.';
        return false;
      }
      ...
      return getList('dbo.QArsMemberRequestHiredList', 'MemberRequestId, MemberRequestName', '', filter).then(
        request => {
          if (request && request.length) {
            me.deminimis.memberRequestName = request[0].memberRequestName;
            return true;
          }
          return false;
        },
    ...
    onMemberRequestIdSearchFill (e) {
      e.filter = "memberId = " + this.deminimis.memberId 
      ...
      return getList('dbo.HrsMember', 'MemberId, MemberName', '', filter).then(
        member => {
          if (member && member.length) {
            me.deminimis.memberName = member[0].memberName;
            return true;
          }
          return false;
        },
    ...
    loadData () {
      ...
      me.getReferences()
        .then( data => {
          ...
          if (me.deminimis.memberDeminimisId < 0) {
            return Promise.resolve(null);
          }
          return me.getDeminimis();
        })
        .then( info => {
          me.stopWait(wait);
          if (info && info.deminimis.length) {
            me.setModels(info);
          } else {
            if (me.deminimis.memberDeminimisId > -1) {
              let message = "Member Deminimis ID '<b>" + me.deminimis.memberDeminimisId + "</b>' not found.";
    ...
    setModels (info) {
      ...
      me.deminimis = me.core.convertDates(info.deminimis[0]);
      me.trxs = info.trxs;
    ...
    getDeminimis () {
      return get('api/member-deminimis/' + this.deminimis.memberDeminimisId);
    },
    ...
    getReferences () {
      ...
      return get('api/references/ars0140');
    },
    ...
    createRecord () {
      ...
      return ajax('api/member-deminimis/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/member-deminimis/' + this.deminimis.memberDeminimisId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/member-deminimis/' + deminimis.memberDeminimisId + this.getDeleteQuery(deminimis.lockId), options);
    },
    ...
    getApiPayload () {
      ...
      Object.assign(deminimis, me.deminimis);
      deminimis.trxs = me.trxs;
    ...
    onLoad () {
      ...
      dc.models.push('deminimis', 'trxs', 'trx',);
      dc.keyField = 'memberDeminimisId';
    ...
    onMemberDeminimisIdChanging (e) {
      e.callback = this.memberDeminimisIdCallback;
    },

    memberDeminimisIdCallback (e) {
      e.message = "Member Deminimis ID '<b>" + e.proposedValue + "</b>' not found.";

      return this.isValidEntity('dbo.ArsMemberDeminimis', 'memberDeminimisId', e.proposedValue).then(
        result => {
          return result;
        }
      );
    ...
    onMemberDeminimisIdLostFocus () {
      const me = this;

      if (!me.deminimis.memberDeminimisId && !me.isModalShown()) {
        me.goTop();
      }
    },

    onMemberDeminimisIdSearchFill (e) {
      e.filter = "MemberId = " + this.deminimis.memberId + " AND MemberRequestId = " + this.deminimis.memberRequestId    
    },


    onMemberDeminimisIdSearchResult (result) {
      if (!result) { return; }
      ...
      me.deminimis.memberDeminimisId = data.memberDeminimisId;
      me.replaceUrl();
      me.loadData();
    },
        ...
        (success) => {
          ...
          if (success) {
            if (isNew && typeof success === 'number' && success > 0) {
              me.deminimis.memberDeminimisId = success;
            }
    ...
    onDelete () {
      ...
      getSafeDeleteFlag('ArsMemberDeminimis', me.deminimis.memberDeminimisId)
        .then( safe => {
          if (safe) {
    ...
    onEditTrx (dtl, index) {
      ...
      d.memberDeminimisTrxId = dtl.memberDeminimisTrxId;
    ...
    onDeleteTrx (index) {
      ...
      me.dialog.confirm('Deminimis <b>' + me.trxs[index].payTrxName + '</b> will be removed from the list.<br><br>Continue?', { size: 'md', scheme: 'warning' }).then(
        reply => {
          if (reply === 'yes') {
    ...
    onSubmitTrx() {
      ...
      if (me.isAddingTrx) {
        ...
        me.trxs.push(dtl);
        me.clearTrxPad();
        me.advice.info("Deminimis '" + dtl.payTrxName + "' added to list.", { duration: 5 });
        ...
        dtl.memberDeminimisTrxId = d.memberDeminimisTrxId;

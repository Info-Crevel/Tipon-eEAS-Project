...
<sym-form id="ars0150" :caption="pageName" headerClass="app-form-header" footerClass="border-top-main app-form-footer py-0" bodyClass="app-form-body pb-3">
  ...
  <div class="d-flex justify-between align-items-end fw-96">
    <sym-int v-model="payOut.clientPayOutDetailId" :caption-width="46" :input-width="20" caption="PayOut ID" lookupId="ArsClientPayOut" @lostfocus="onClientPayOutDetailIdLostFocus" @changing="onClientPayOutDetailIdChanging" @changed="onClientPayOutDetailIdChanged" @searchfill="onClientPayOutDetailIdSearchFill" @searchresult="onClientPayOutDetailIdSearchResult"></sym-int>
  ...
  <sym-combo v-model="payOut.payTrxCode" :caption-width="46" :input-width="50" caption="Trx Name" display-field="payTrxName" :datasource="payTrxs" @changing="onPayTrxCodeChanging" @changed="onPayTrxCodeChanged"></sym-combo>
   <sym-text v-model="payOut.payOutSheetFormula" :caption-width="46" :input-width="70" caption="Formula (A,B,C,D, etc):"></sym-text> 
  <!-- <sym-check v-model="payOut.orgFlag" :caption-width="46" caption="Shouldered By Coop" @changed="onOrgFlagChanged"></sym-check>   -->
  <!-- <sym-dec v-model="payOut.adminFee" v-show="!isOrg" :caption-width="46" :input-width="30" caption="Admin Fee ( % )" ></sym-dec>           -->
    ...
    <sym-tab title="Basic Rate (A)">
      <sym-table class="table-select striped-odd bill hover mb-0" colorClass="light">
        <sym-table-header slot="header">
          ...
          <sym-table-body slot="body" v-show="payOut.clientPayOutDetailId !=0">
...
>
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="ars0150C" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
...
export default {
  ...
  methods: {
    ...
    onPayTrxCodeChanging (e) {
      e.message = "Trx Code '<b>" + e.proposedValue + "</b>' not found / acceptable.";
      e.callback = this.payTrxCodeCallback;
    },

 payTrxCodeCallback(e) {
  const me = this;
  let filter = "PayTrxCode='" + e.proposedValue + "' AND ClientPayGroupId=" + me.payOut.clientPayGroupId;
  return getList('dbo.ArsClientPayOut', 'ClientPayGroupId,PayTrxCode', '', filter).then(
    info => {
      if (info && info.length) {
        // If payOut has a property 'payTrxCode' and you want to update it:
        if (me.payOut.payTrxCode === e.proposedValue) {
          me.payOut.payTrxCode = info[0].payTrxCode;
        } else {
          // Or, if you want to just update it anyway:
          me.payOut.payTrxCode = info[0].payTrxCode;
        }
        return true;
      }
    ...
    if (allSelected) {
      ...
      this.dayTypeSheet.forEach(a => {
        ...
        if (!exists) {
          this.dayTypes.push({
            clientBillingDetailId: this.payOut.clientBillingDetailId,
...
,
    ...
    onToggleDayTypeIdSelection (dayTypes) {
      ...
      if (index > -1) {
        ...
        me.dayTypes.push({
          clientPayOutDetailId: me.payOut.clientPayOutDetailId,
    ...
    if (allSelected) {
      ...
      this.deminimisList.forEach(a => {
        ...
        if (!exists) {
          this.deminimis.push({
            clientPayOutDetailId: this.payOut.clientPayOutDetailId,
...
,
    ...
    getTargetPath() {
      ...
      if (me.payOut.clientPayGroupId) {
        q.clientPayGroupId = me.payOut.clientPayGroupId;
        me.refClientPayGroupId = q.clientPayGroupId;
      }
      
      if (me.payOut.clientPayOutDetailId) {
        q.clientPayOutDetailId = me.payOut.clientPayOutDetailId;
      }
    ...
    syncValues(p, q) {
      ...
      if ("clientPayGroupId" in q && me.core.isInteger(q.clientPayGroupId)) {
        me.payOut.clientPayGroupId = parseInt(q.clientPayGroupId);     
        me.refClientPayGroupId = parseInt(q.clientPayGroupId);
      }
      
      if ("clientPayOutDetailId" in q && me.core.isInteger(q.clientPayOutDetailId)) {       
        me.payOut.clientPayOutDetailId = parseInt(q.clientPayOutDetailId);
      }
    ...
    onPayTrxCodeChanged (newValue) {
      ...
      if (o) {
        me.payOut.payOutSheetFormula = o.trxFormula;
      }
    },

    onOrgFlagChanged(newValue){
      if (newValue===true) {
      this.payOut.adminFee = 0;
      }
   },
    ...
    onEditDayType (dtl, index) {
      ...
      d.payOutDayTypeId = dtl.payOutDayTypeId;
    ...
    onSubmitDayType() {
      ...
      if (me.isAddingDayType) {
        ...
        dtl.payOutDayTypeId = d.payOutDayTypeId;
    ...
    loadData () {
      const
        ...
        wait = me.wait();
          ...
          this.payOut.clientPayGroupId = me.refClientPayGroupId;
      ...
      me.getReferences()
        .then( data => {
          ...
          if (me.payOut.clientPayOutDetailId < 0) {
            return Promise.resolve(null);
          }
          return me.getClientPayOut();
        })
        .then( info => {
          me.stopWait(wait);
          if (info && info.payOut.length) {
            me.setModels(info);
          } else {
            if (me.payOut.clientPayOutDetailId > -1) {
              let message = "PayOut ID '<b>" + me.payOut.clientPayOutDetailId + "</b>' not found.";
    ...
    setModels (info) {
      ...
      me.payOut = info.payOut[0];
      me.deminimis = info.payOutDeminimis;
      me.allowances = info.payOutAllowances;
      me.basicRate = info.payOutBasicRate;
    ...
    refreshOldRefs () {
      ...
      me.oldPayOut = JSON.stringify(me.payOut);
    ...
    getClientPayOut () {
      return get('api/client-pay-group-payOuts/' + this.payOut.clientPayOutDetailId);
    },
    ...
    getReferences () {
      ...
      return get('api/references/ars0150');
    },
    ...
    createRecord () {
      ...
      return ajax('api/client-pay-group-payOuts/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/client-pay-group-payOuts/' + this.payOut.clientPayOutDetailId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/client-pay-group-payOuts/' + payOut.clientPayOutDetailId + this.getDeleteQuery(payOut.lockId), options);
    },
    ...
    getApiPayload () {
      const
        me = this,
        payOut = {};
        console.log('me.refClientPayGroupId', me.refClientPayGroupId)
        me.payOut.clientPayGroupId = me.refClientPayGroupId;
        Object.assign(payOut, me.payOut);
        payOut.deminimis = me.deminimis;
        payOut.allowances = me.allowances;
        payOut.basicRate = me.basicRate;
        payOut.dayTypes = me.dayTypes;
        console.log('AAA', me.payOut)
      return payOut;
    },
    ...
    onLoad () {
      ...
      dc.models.push('payOut', 'deminimis', 'allowances','basicRate','dayTypes','dayType');
      dc.keyField = 'clientPayOutDetailId';
    ...
    onClientPayOutDetailIdChanging (e) {
      e.callback = this.clientPayOutDetailIdCallback;
    },

    clientPayOutDetailIdCallback (e) {
      const me = this;
      let filter = "ClientPayOutDetailId='" + e.proposedValue + "' AND ClientPayGroupId = " + this.refClientPayGroupId;
      return getList('dbo.ArsClientPayOut', 'ClientPayOutDetailId, ClientPayGroupId', '', filter).then(
        payOut => {
          if (payOut && payOut.length) {
            me.payOut.clientPayOutDetailId = payOut[0].clientPayOutDetailId;
            return true;
          }
          return false;
        },
    ...
    onClientPayOutDetailIdLostFocus () {
      const me = this;

      if (!me.payOut.clientPayOutDetailId && !me.isModalShown()) {
        me.goTop();
      }
    },

    onClientPayOutDetailIdSearchFill (e) {
      e.filter = "ClientPayGroupId = " + this.refClientPayGroupId;
    },


    onClientPayOutDetailIdSearchResult (result) {
      if (!result) { return; }
      ...
      me.payOut.clientPayOutDetailId = data.clientPayOutDetailId;
      me.replaceUrl();
      me.loadData();
    },
    ...
    onSubmit (nextRoute) {
      ...
      // if (!me.payOut.orgFlag && me.payOut.adminFee==0) {
        ...
        (success) => {
          ...
          if (success) {
            if (isNew && typeof success === 'number' && success > 0) {
              me.payOut.clientPayOutDetailId = success;
            }
    ...
    onDelete () {
      ...
      getSafeDeleteFlag('ArsClientPayOut', me.payOut.clientPayOutDetailId)
        .then( safe => {
          if (safe) {
    ...
    hasChanges () {
      ...
      if (JSON.stringify(me.payOut) !== me.oldPayOut) { return true; }

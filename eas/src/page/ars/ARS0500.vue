...
<section class="container p-0" :key="ts">
   <sym-form id="ars0500" :caption="pageName" cardClass="curved-0" headerClass="app-form-header" footerClass="border-top-main app-form-footer darken-2 py-0" bodyClass="app-form-body pb-3 ">
  ...
  <div>
    <div class="d-flex">
      <div class="d-flex justify-between align-items-end fw-118 ">
        <sym-int v-model="trx.trxId" :caption-width="42" :input-width="34" caption="Transaction ID" lookupId="FinTrxReceivable" @lostfocus="onTrxIdLostFocus" @changing="onTrxIdChanging" @changed="onTrxIdChanged" @searchresult="onTrxIdSearchResult"></sym-int>
    ...
    <div class="d-flex gap-1">
      <sym-int v-model="trx.clientId" :caption-width="42" :input-width="34" caption="Client ID" lookupId="ArsClient" @changing="onClientIdChanging" @searchresult="onClientIdSearchResult" @changed="onAmountChanged"></sym-int>
      <sym-text v-model="trx.clientName" class="w-43"></sym-text>
    </div>
    ...
    <div class="grid cols-4 gap-1">
      ...
      <sym-dec class="text-right"  v-model="trx.amount" align="bottom"  :caption-width="42" :input-width="50" caption="Amount" @changed="onAmountChanged"></sym-dec>    
    </div>
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="ars0500A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="ars0500B" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
...
<div class="to-print">
  <table class="mt-2 white text-charcoal mb-0">
    <tbody>
      ...
      <tr class="doc-header">
        <td colspan="3" class="text-center display-9 app-bold doc-title">RECEIVABLE VOUCHER</td>
      ...
      <tr class="doc-header">
        <td colspan="3" class="text-left sm-1 pt-0">Client ID: {{ trx.clientId }} &nbsp;&nbsp;  Client Name: {{ trx.clientName }}</td>
      </tr>
...
export default {
  ...
  data () {
    return {
      ...
      trx: {
        ...
        clientName: '',
        details: '',
        receivableTradeAccount: '',
        clientAccount: '',
        clientAccountName: '',
        receivableTradeAmount: 0,
        amount: 0,
        arsLockId: '',
      },
  ...
  this.trxDetails = [];
  const tradeAccount = this.control[0].receivableTradeAccount || '';
  const tradeAccountName = this.control[0].receivableTradeAccountName || 'NO ACCOUNT';
  ...
  if (this.trx.clientAccount > 0) {
    this.trxDetails.push({
      accountId: this.trx.clientAccount || '',
      accountName: this.trx.clientAccountName || 'NO ACCOUNT',
      debitAmount: amount,
      creditAmount: 0
    });
  }
  ...
  this.refreshTotals();
    ...
    clientIdCallback (e) {
      ...
      return getList('dbo.QArsClient', 'ClientId, ClientName, AccountId, AccountName', '', filter).then(
        data => {
          if (data && data.length) {
            me.trx.clientName = data[0].clientName;
            me.trx.clientAccount = data[0].accountId;
            me.trx.clientAccountName = data[0].accountName;
            return true;
          }
          return false;
        },
    ...
    onClientIdSearchResult (result) {
      ...
      trx.clientId = data.clientId;
      trx.clientName = data.clientName;
      trx.clientAccount = data.accountId;
      trx.clientAccountName = data.accountName;
    ...
    getTrx () {
      return get('api/receivable/' + this.trx.trxId);
    },

    getReferences () {
      const me = this;

      if (me.control.length) {
        return Promise.resolve(true);
      }
      return get('api/references/ars0500');
    },

    getChangeLog () {
      return get('api/receivable/' + this.trx.trxId + '/log');
    },
    ...
    createRecord () {
      ...
      return ajax('api/receivable/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/receivable/' + this.trx.trxId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/receivable/' + trx.trxId + '/' + trx.lockId, options);
    },

// EMDS Disbursements Journal Entry

<template>
<section class="container p-0" :key="ts">
<sym-form id="gls0070" :caption="pageName" cardClass="curved-0" headerClass="frs-form-header" footerClass="border-top-main frs-form-footer darken-2 py-0" bodyClass="frs-form-body pb-3">
  ...
  <div>
    <!-- <div class="d-flex justify-between align-items-end fw-118 ">
      <sym-int v-model="trx.trxId" :caption-width="42" :input-width="34" caption="Transaction ID" lookupId="FinTrxAdaDisbursement" @lostfocus="onTrxIdLostFocus" @changing="onTrxIdChanging" @changed="onTrxIdChanged" @searchresult="onTrxIdSearchResult"></sym-int>
    ...
    <div class="d-flex">
      <div class="d-flex justify-between align-items-end fw-118 ">
        <sym-int v-model="trx.trxId" :caption-width="42" :input-width="34" caption="Transaction ID" lookupId="FinTrxEmdsDisbursement" @lostfocus="onTrxIdLostFocus" @changing="onTrxIdChanging" @changed="onTrxIdChanged" @searchresult="onTrxIdSearchResult"></sym-int>
        <div class="buttons d-inline">
          <button class="btn-new warning-light border-main justify-between fw-28 shadow-light mb-2" :tabindex="-1" @mousedown.prevent @click="onNew"><i class="fa fa-file-o"></i>New</button>
        </div>
      </div>
    ...
    <sym-combo v-model="trx.fundClusterId" :caption-width="42" :input-width="118" caption="Fund Cluster" display-field="fundClusterName" :datasource="fundClusters" v-if="isFundClusterVisible"></sym-combo>
    ...
    <sym-combo v-model="trx.costCenterId" :caption-width="42" :input-width="44" caption="Cost Center" display-field="costCenterShortName" :datasource="costCenters" v-if="isCostCenterVisible"></sym-combo>
    <!-- <sym-memo v-model="trx.particulars" :input-width="278" caption="Particulars"></sym-memo> -->
    ...
    <sym-text v-model="trx.reference" :caption-width="42" :input-width="44" caption="RADAI #"></sym-text>
    <!-- <sym-text v-model="trx.disburserName" :caption-width="42" :input-width="108" caption="Disbursing Officer" lookupId="DbsDisburser" @searchresult="onDisburserNameSearchResult"></sym-text> -->

    <!-- <sym-int v-model="trx.payeeId" :caption-width="42" :input-width="34" caption="Payee ID" lookupId="DbsPayee" @changing="onPayeeIdChanging" @searchresult="onPayeeIdSearchResult"></sym-int>
    <sym-text v-model="trx.payeeName" :caption-width="42" :input-width="236" caption="Payee Name"></sym-text> -->

    <div class="d-flex gap-1">
      <sym-int v-model="trx.payeeId" :caption-width="42" :input-width="34" caption="Payee ID" lookupId="DbsPayee" @changing="onPayeeIdChanging" @searchresult="onPayeeIdSearchResult"></sym-int>
      <sym-text v-model="trx.payeeName" class="w-100"></sym-text>
    </div>

    <!-- <sym-text v-model="trx.adaSeriesFrom" :caption-width="42" :input-width="54" caption="ADA Ser. # (From)"></sym-text> -->
    <!-- <sym-text v-model="trx.adaSeriesTo" :caption-width="42" :input-width="54" caption="ADA Ser. # (To)"></sym-text> -->

    <!-- <sym-text v-model="trx.adaSeries" :caption-width="42" :input-width="44" caption="LDDAP-ADA #"></sym-text> -->
    <sym-text v-model="trx.adaSeries" :caption-width="42" :input-width="44" caption="Check / Serial #"></sym-text>

    <div class="d-flex gap-1">
      <sym-int v-model="trx.requestSignatoryId" :caption-width="42" :input-width="34" caption="Prepared By" lookupId="DbsSignatory" @changing="onRequestSignatoryIdChanging" @searchfill="onRequestSignatoryIdSearchFill" @searchresult="onRequestSignatoryIdSearchResult"></sym-int>
      <!-- <sym-text v-model="trx.requestSignatoryName" :input-width="190"></sym-text> -->
      <sym-text v-model="trx.requestSignatoryName" class="w-100"></sym-text>
    </div>
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="gls0070A" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
      <div class="detail-editor-boxes">
        <sym-text v-model="detail.accountId" caption="Account ID" align="bottom" lookupId="GlsChartDetail" @changing="onAccountIdChanging" @searchresult="onAccountIdSearchResult"></sym-text>
  ...
  <div class="board p-1 mb-0 w-100">
    <form id="gls0070B" class="curved-bottom border-dark marianblue p-3" @submit.prevent>
...
<sym-modal
  ...
  :dismissible="true"
  :closeOnBackButton="false"
  title="Journal Entry Change Log"
  headerClass="frs-form-header"
  dismissButtonClass="danger"
>
...
<div class="to-print">
  <table class="mt-2 white text-charcoal mb-0">
    <tbody>
      ...
      <tr class="doc-header">
        <td colspan="3" class="text-center display-9 frs-bold doc-title">JOURNAL ENTRY VOUCHER</td>
        <td colspan="2">No. <span class="lg-1 frs-bold">{{ docId }}</span></td>
      </tr>
      <tr class="doc-header">
        <td colspan="3" class="text-center lg-2 frs-bold">{{ sym.sysInfo.siteName }}</td>
      ...
      <tr class="doc-header">
        ...
        <td colspan="2" class="text-center frs-bold">Amount</td>
      </tr>

      <tr class="col-header frs-bold align-top">
        <td>Cost Ctr</td>
      ...
      <tr>
        <td colspan="3" class="text-right frs-bold">Total</td>
        <td class="text-right">{{ core.toDecimalFormat(trx.debitTotal) }}</td>
        <td class="text-right">{{ core.toDecimalFormat(trx.creditTotal) }}</td>
        <td></td>
      </tr>  
      ...
      <tr class="align-top">
        ...
        <td>
          ...
          <span class="lg-1 frs-bold">{{ trx.requestSignatoryName }}</span> <br>
          <span>{{ trx.requestSignatoryPosition }}</span>
        </td>
        <td colspan="3">
          Certified Correct: <br><br><br><br>
          <span class="lg-1 frs-bold">{{ trx.signatoryName }}</span> <br>
...
export default {
  ...
  data () {
    return {
      trx: {
        ...
        // postUserName: '',
        // postUserPosition: '',
        requestSignatoryId: 0,
        requestSignatoryName: '',
        requestSignatoryPosition: '',
        signatoryId: 0,
        signatoryName: '',
        signatoryPosition: '',
        lockId: '',
  ...
  computed: {
    ...
    docId () {
      return this.trx.documentId + ' (EMDSDJ)';
    },
  ...
  methods: {
      ...
      me.getReferences()
        ...
        .then( info => {
          ...
          if (info && info.trx.length) {
            ...
            me.trx.journalId = 6;               // EMDS Disbursements Journal
            me.trx.fundClusterId = '01';        // Regular Agency Fund
            me.trx.trxStatusId = 1;             // Posted
            me.trx.costCenterId = 255;          // this Agency
            me.trx.postUserId = me.sym.userInfo.userId;
            me.trx.signatoryId = appConfig.journalVoucherSignatoryId;   // head, acctg unit
    ...
    getTrx () {
      return get('api/trans-emdsd/' + this.trx.trxId);
    },
    ...
    getReferences () {
      ...
      return get('api/references/gls0070');
    },

    getChangeLog () {
      return get('api/trans-emdsd/' + this.trx.trxId + '/log');
    },
    ...
    createRecord () {
      ...
      return ajax('api/trans-emdsd/' + currentUserId, options);
    },
    ...
    modifyRecord () {
      ...
      return ajax('api/trans-emdsd/' + this.trx.trxId + '/' + currentUserId, options);
    },
    ...
    deleteRecord () {
      ...
      return ajax('api/trans-emdsd/' + trx.trxId + '/' + trx.lockId, options);
    },
    ...
    trxIdCallback (e) {
      ...
      let filter = 'TrxId=' + e.proposedValue + ' AND JournalId=6'
    ...
    payeeIdCallback (e) {
      ...
      return getList('dbo.DbsPayee', 'PayeeId, PayeeName', '', filter).then(
        data => {
          if (data && data.length) {
    ...
    onSelectTemplate () {
      ...
      let options = {
        triggerControl: me,
        filter: 'JournalId = 6',      // EMDS disbursements
        sort: '',
        size: '',
        dismissible: true,
        searchValue: ''

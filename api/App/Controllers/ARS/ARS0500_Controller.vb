<RoutePrefix("api")>
Public Class ARS0500_Controller
    Inherits ApiController

    Private ReadOnly _databaseId As String = "sys"

    <SymAuthorization("GetReferences_ARS0500")>
    <Route("references/ars0500")>
    <HttpGet>
    Public Function GetReferences_ARS0500() As IHttpActionResult

        Try
            Using _direct As New SqlDirect("web.ARS0500_References")
                With _direct

                    Using _dataSet As DataSet = _direct.ExecuteDataSet()
                        With _dataSet
                            .Tables(0).TableName = "control"
                        End With
    ...
    <SymAuthorization("GetReceivableTrx")>
    <Route("receivable/{trxId}")>
    <HttpGet>
    Public Function GetReceivableTrx(trxId As Integer) As IHttpActionResult
        ...
        Try
            Using _direct As New SqlDirect("web.ARS0500")
                With _direct
    ...
    <SymAuthorization("GetReceivableTrxLog")>
    <Route("receivable/{trxId}/log")>
    <HttpGet>
    Public Function GetReceivableTrxLog(trxId As Integer) As IHttpActionResult
        ...
        Try
            Using _direct As New SqlDirect("web.ARS0500_Log")
                With _direct
                    .AddParameter("TrxId", trxId)
    ...
    <SymAuthorization("CreateReceivableTrx")>
    <Route("receivable/{currentUserId}")>
    <HttpPost>
    Public Function CreateReceivableTrx(currentUserId As Integer, <FromBody> trx As ReceivableTrxBody) As IHttpActionResult
        ...
        Try
            ...
            trx.TrxTypeId = 11 'Accounts Payable
            ...
            If trx.DocumentId = "< NEW >" Then
                trx.DocumentId = AppLib.GetNextTrxTypeSequence(NextSeqId.AccountsReceivable)
            End If

            '
            ' Load proposed values from payload
            '
            Dim _finTrx As New ArsFinTrx
            Dim _finTrxDetailList As New FinTrxDetailList
            Dim _arsTrx As New ArsTrx
    ...
    <SymAuthorization("ModifyReceivableTrx")>
    <Route("receivable/{trxId}/{currentUserId}")>
    <HttpPut>
    Public Function ModifyReceivableTrx(trxId As Integer, currentUserId As Integer, <FromBody> trx As ReceivableTrxBody) As IHttpActionResult
        ...
        Try
            ...
            '
            Dim _finTrxOld As New ArsFinTrx
            Dim _finTrxDetailListOld As New FinTrxDetailList
            Dim _arsTrxOld As New ArsTrx
            Dim _result As Results.OkNegotiatedContentResult(Of DataSet) = CType(Me.GetReceivableTrx(trxId), Results.OkNegotiatedContentResult(Of DataSet))

            Using _dataSet As DataSet = _result.Content
                Dim _row As DataRow = _dataSet.Tables("trx").Rows(0)
                Me.LoadArsFinTrx(_row, _finTrxOld)
            ...
            With _arsTrxOld
                ...
                .ClientId = _finTrxOld.ClientId
                .Amount = _finTrxOld.Amount
                .LockId = _finTrxOld.ArsLockId
            ...
            Dim _isArsTrxChanged As Boolean = Me.HasArsTrxChanges(_arsTrxOld, _arsTrx)

            If Not _isFinTrxChanged AndAlso _addDetailCount = 0 AndAlso _removeDetailCount = 0 AndAlso _editDetailCount = 0 AndAlso Not _isArsTrxChanged Then
                '
                ' No changes; just return current transaction
                ' 
                Return Me.GetReceivableTrx(trxId)
            End If
            ...
            Try
                ...
                Try
                    ...
                    If _isArsTrxChanged Then
                        Me.UpdateArsTrx(_arsTrx)
                    End If
    ...
    <SymAuthorization("RemoveReceivableTrx")>
    <Route("receivable/{trxId}/{lockId}")>
    <HttpDelete>
    Public Function RemoveReceivableTrx(trxId As Integer, lockId As String) As IHttpActionResult
    ...
    Private Sub LoadArsFinTrx(trx As ReceivableTrxBody, finTrx As ArsFinTrx)

        DataLib.ScatterValues(trx, finTrx)

    End Sub

    Private Sub LoadArsFinTrx(row As DataRow, trx As ArsFinTrx)
    ...
    End Sub
    Private Sub InsertFinTrx(trx As ArsFinTrx)
        ...
        With DataCore.Command
            ...
            Me.AddInsertUpdateParamsArsFinTrx(trx)
    ...
    End Sub
    Private Sub InsertArsTrx(ars As ArsTrx)
        ...
        With DataCore.Command
            .CommandText = DataLib.BuildInsertCommandText("dbo.ArsTrx", ars, _excludedFields)
            .CommandType = CommandType.Text

            Me.AddInsertUpdateParamsArsTrx(ars)
    ...
    Private Sub UpdateFinTrx(trx As ArsFinTrx)
    ...
    End Sub
    Private Sub UpdateArsTrx(trx As ArsTrx)
        ...
        With DataCore.Command
            .CommandText = DataLib.BuildUpdateCommandText("dbo.ArsTrx", trx, _keyFields, _excludedFields)
            .CommandType = CommandType.Text

            Me.AddInsertUpdateParamsArsTrx(trx)
            .Parameters.AddWithValue("@LockId", Convert.FromBase64String(trx.LockId)).DbType = DbType.Binary
    ...
    Private Sub AddInsertUpdateParamsArsFinTrx(trx As ArsFinTrx)
    ...
    Private Sub AddInsertUpdateParamsArsTrx(ars As ArsTrx)

        With DataCore.Command.Parameters
            .Clear()
            .AddWithValue("@TrxId", ars.TrxId)
            .AddWithValue("@ClientId", ars.ClientId)
            .AddWithValue("@Amount", ars.Amount)
        End With
    ...
    Private Function HasFinTrxChanges(oldRecord As ArsFinTrx, newRecord As ArsFinTrx) As Boolean
    ...
    Private Function HasArsTrxChanges(oldRecord As ArsTrx, newRecord As ArsTrx) As Boolean

        With oldRecord
            If .ClientId <> newRecord.ClientId Then Return True
...
End Class
Public Class ReceivableTrxBody
   Inherits ArsFinTrx
